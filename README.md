...
| Skills | — | Twenty-three, in three groups. The four the agents follow are in the column to the left. Twelve more a session loads on its own when the work calls for them — `agent-contract`, `audit`, `build-fix`, `checkpoint`, `commit-pr-convention`, `confidence-check`, `debug`, `evidence-check`, `feature-planner`, `gap-analysis`, `learn`, `verify`. Seven you invoke by name; they are in the cheat sheet below |
