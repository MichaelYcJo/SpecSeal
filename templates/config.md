...
- **The field names, section headings and vocabulary a checker or a pinned
  ...
  `Fixes checked by`, `Contract changes`, `New units`, `Needs a fix`,
  `Loses a record or crashes` and its `Pass` checkbox; its `## Verdicts`,
  `## Executed probes`, `## Inherited coordinates` and `## Deferred`
  headings, and the `Verdict` column of the first; the verdict words `fixed`,
  `answered`, `withdrawn`, `not a defect` and `agreed, fixed`; `round-N`,
  `none`, `no fixes to check` and `nobody` in `nobody — <why>`; an
  `overview.md`'s `## Not verified` heading with its `Item` and
  `Who must answer` columns. A translated field name is not a translation, it
  is a broken gate.
