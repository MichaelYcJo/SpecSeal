...
| Broad gate | <`not yet`, or one entry per full-suite run, NEWEST FIRST — each `<sha> against <base>`, the SHA the run happened at and the base it was compared against, with every earlier run kept behind the newest as `; earlier run: <sha> against <base>` (#174). WRITTEN by `round_record.py seal`, which the `sealer` reaches through `broad-gate --record <item>` on a green run — it writes the new entry in front and keeps what the cell held, so a first seal is one entry and a re-seal is two, and a run the newest entry already records — the same commit against the same base — replaces that entry rather than duplicating it, while a run at that commit against another base is kept behind the new entry as any earlier run is — `close --broad-gate` writes the same cell through the same path where fixes and the gate land in one pass, so neither writer erases a run the other kept. READ by `chain_check.py` on the LAST record at a ready pull request, which takes the first SHA-shaped word as the run: `not yet` — or no row at all — fails as the run that never happened, and a newest entry this record's own `Target SHA` descends from fails as the run spent before the round it was meant to seal. A value it cannot parse is reported rather than failed, and work items begun before `chain_check.GATE_FROM` print instead of failing> |
| Fixes checked by | <`round-<N>`, a LATER round · `no fixes to check` · `nobody — <why>`> |
| Fix range | <`` `<a>..<b>`, N commits `` — the range this round's fixes were measured over, with BOTH ends written as commits somebody can open. WRITTEN by `round_record.py close` from its own `--range`, which refuses an end that is not a commit: `HEAD`, `@`, a branch and a tag all resolve today and name something else tomorrow, so a record stating one says a different set of commits every time it is read. READ by `chain_check.fix_range`, which re-resolves both ends and compares the count against the tree — ends this repository cannot see are reported rather than failed, because a squash keeps none of a feature branch's own commits. `none` is the honest value for a round that commissioned no fixes, and the value this row starts at — but the starting value beside a `round-N` in `Fixes checked by` is refused, as the two surface rows' is: a later round opened the fixes, so they exist and were measured over some range. Work items begun before `chain_check.RANGE_FROM` print instead of failing> |
