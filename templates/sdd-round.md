...
| Fix range | <`` `<a>..<b>`, N commits `` — the range this round's fixes were measured over, with BOTH ends written as commits somebody can open. WRITTEN by `round_record.py close` from its own `--range`, which refuses an end that is not a commit: `HEAD`, `@`, a branch and a tag all resolve today and name something else tomorrow, so a record stating one says a different set of commits every time it is read. READ by `chain_check.fix_range`, which re-resolves both ends and compares the count against the tree — ends this repository cannot see are reported rather than failed, because a squash keeps none of a feature branch's own commits. `none` is the honest value for a round that commissioned no fixes, and the value this row starts at. Work items begun before `chain_check.RANGE_FROM` print instead of failing> |
