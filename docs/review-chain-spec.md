...
| `no`, with two or more later round records counted — the count stopping at the first that reopened the run or closed on a fix, that record included | **fails**, naming the exit. One later record is the verifying round; a second is the run carrying on past its own stopping rule. A record whose own verdicts say `fixed` stops the count exactly as a reopening does — and a reopening in second place does not excuse the quiet round in first |
