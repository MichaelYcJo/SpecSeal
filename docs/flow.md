...
- [ ] **#150 — a round record written after the fixes it commissioned looks exactly like one written before them.** Measured twice in this release: the reviewer's drafted text lived only in a report and the next segment rebuilt it, once with the record missing and once with the record present but not carrying what it said it verified. Before #145, because a boundary is easier to draw around records that exist when they are supposed to.
...
- [ ] #153 — scoping `evidence-check` to a work item's fragment hides every row the branch broke in the shared ledger. #137's three rounds and two fix passes all reported a clean ledger; its broad gate found fifteen drifted rows and one broken claim.
