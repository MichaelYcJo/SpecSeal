...
def test_a_fence_line_with_an_info_string_inside_a_fix_does_not_end_it(repo):
    """Round 1's 🟡 1 of work item 1790260566 (#491, one walk over). A fix to
    a markdown document shows a fence line with an info string inside its own
    block, and CommonMark 4.5 says a closer carries nothing after its run, so
    that line is content. `fenced_after` closed on it: the record lost the
    lines after it and copied the prose between two fixes as if it were
    fenced. Seen red at `f754eafb`."""
    declared(repo)
    first = "```text\na\n```text\nb\n```\n"
    second = "```text\nc\n```text\nd\n```\n"
    fixes = f"{first}\nprose that is not a fence\n\n{second}"
    code, out, text = generate(repo, report_text=report(fixes=fixes))
    assert code == 0, out
    section = paste_ready(text)
    assert first.strip() in section, text
    assert second.strip() in section, text
