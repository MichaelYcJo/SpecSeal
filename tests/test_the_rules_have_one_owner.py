...
carriers; the link tables below hold the links phase 4a's files carry, and a
later phase extends them rather than writing a second pin file. The four
reach-backs are pinned beside the rules: the sections that told the
orchestrator to fill `Fixes checked by`, the fix surface and `Ran by` by
hand now name the subcommand that fills them.
