...
| **A spawn's prompt is classified as a command line in the whole-run reading, so a prompt naming a test runner reads as a test run.** `load` writes a call carrying no `command` field as a JSON dump of its whole input, and a spawn's input is mostly its prompt; the family classifier and the repeat groups then read that string as a command. Two consequences, both measured while building #145's cycle rows: a prompt naming `pytest` outside a heredoc is charged to the `test` family, and two prompts differing only after a `|` — which is what a markdown table makes of every real spawn prompt — collapse into one repeat group and read as a check re-run for a result already in hand. It is FIXED inside cycle rows, which pass a delegated-tool set, and deliberately left in the whole-run path, because that path must go on printing what every published reading was taken against — 0.9.4 exists because three meter numbers moved without anything saying so. **What needs a person is which side pays**: correcting the whole-run reading moves numbers already published and wants the same marking decision #200 needed, and leaving it means the two readings of one transcript disagree about its repeats with nothing on either page saying why | the repository owner |
