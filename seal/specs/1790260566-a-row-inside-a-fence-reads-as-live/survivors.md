...
| seal/follow-up.md | What needs a person is whether the rule is *every module with a `__main__`* or something narrower | Another follow-up row, about which `.github/scripts/` entry points owe `console.to_utf8()`; it shares only the phrase about a change to what a gate reads carrying CONTRIBUTING.md's higher bar with the removed row, and is true as it stands |
