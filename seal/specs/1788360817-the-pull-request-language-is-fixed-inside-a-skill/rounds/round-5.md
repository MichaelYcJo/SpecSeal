...
| PR | none yet |
| Broad gate | not yet |
| Fixes checked by | nobody — the fixes are not yet written; round 6 verifies them and this cell is set to it then |
...
| 2 | `-z` turns the quoting off by itself and `core.quotePath=false` changes nothing while it is there, where the comment credits the second and calls `-z` what makes it safe. Keeping the argument is harmless; the comment sends whoever prunes one of them at the wrong one | `tests/…:751-756` | open, comment only | reviewer executed four variants: no arguments C-escapes, `-z` alone does not, `core.quotePath=false` alone does not, both together do not. `split("\0")` leaves one empty trailing entry and `if name.strip()` drops it |
...
| reviewer: four `git ls-files` quoting variants | `-z` alone turns the quoting off; `core.quotePath=false` alone does not |
