...
| 14 | a prose document whose name is not ASCII, and one whose name holds a space, both reach the corpus — so a template only such a document names is not reported unreachable. Added to the existing `test_the_templates_check_reads_prose_only_and_descends`, not as a case of its own | round 5 finding 3 | ⚠️ planted in part — it pins that `unreachable_templates` does not DROP such a document once it arrives, and not that the listing produces it. Measured: with the `-z` reverted on the corpus listing, all 66 cases still pass. Pinning that half needs a listing that takes a root, which is a new unit and this pass adds none |
