...
| 14 | a prose document whose name is not ASCII, and one whose name holds a space, both reach the corpus — so a template only such a document names is not reported unreachable. Added to the existing `test_the_templates_check_reads_prose_only_and_descends`, not as a case of its own | round 5 finding 3 | ⬜ |
