...
| 5 | a config naming a language outside `LANGUAGE_CODES` does not raise out of the mirror case | round 2 🔴 1 | ⬜ |
| 6 | `configured_language` finds a config under `<root>/.git/seal/` as well as `<root>/seal/` | round 2 🟡 2 | ⬜ |
| 7 | both the skill and the template resolve the mirror's home and exclude the git directory | round 2 🟡 3 | ⬜ |
| 8 | the templates check reads only prose documents and descends into subdirectories | round 2 🟡 4 | ⬜ |
| 9 | a separator below the rows, and a repeated `\| Item \| Value \|` header, both end the table | round 2 🟡 5 | ⬜ |
| 10 | every committed mirror's code is one this file knows — `pr.kr.md` is caught | round 2 🟡 7 | ⬜ |
