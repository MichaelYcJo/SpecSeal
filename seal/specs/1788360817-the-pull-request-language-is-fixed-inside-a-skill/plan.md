...
| 7 | Round 5's three findings, under round 4's constraint still standing: **add no new unit, not even a test case.** The corpus listing gets the three arguments round 4 applied to only one of two sibling calls; the two comments come down to what their arguments actually do; and the memo gains round 5's stronger reason for the abandoned skill-side pin — that check is wrong to build rather than merely expensive | 66 cases in the file, unchanged in number for the second pass running, and 431 in the slice; the listing defect reproduced on a scratch repository whose prose is `docs/안내.md` and `docs/two words.md` — both templates unreachable before, none after; `evidence-check --strict .` unscoped, `357 ok · 0 drifted · 0 broken`. **Measured and recorded rather than claimed:** a scratch copy with the corpus `-z` reverted still passes all 66, so row 14 pins half of what it names | `eb18a40` |
