...
| **The fourth box's row name** — `Attendance`, values `nobody at the keyboard` · `somebody may be asked` | Names neither a party nor an agent, and the value answers what a reader six months later actually asks | **Chosen.** A naming call the owner can overturn at this approval; overturning it changes one constant, one template row and their cases |
