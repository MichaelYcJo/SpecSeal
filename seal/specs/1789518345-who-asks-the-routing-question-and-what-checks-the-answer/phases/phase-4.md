...
| `test_the_smith_carries_both_halves_rather_than_only_citing_them`'s requirement that `agents/smith.md` carry the routing vocabulary | `test_the_smith_carries_its_own_half_and_not_the_questions`, which requires its absence, plus `test_the_smith_claims_no_act_another_definition_owns` for the three acts by name |
