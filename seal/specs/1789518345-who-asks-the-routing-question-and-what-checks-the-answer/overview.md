...
| What a survivor check bounds | `survivors.md`'s range row claimed *no hook and no shipped document still says the batch is the framer's*, checked against everything the tool reported | **true of the tool's output and false of the tree** | `survivor-check` matches the sentences a range REMOVED, verbatim. Three lines in this branch's own tests said the routing act was the framer's in words the removal never contained — paraphrases the branch wrote ABOUT its own change — so they were never in the removed set. A survivor check bounds what a removal left behind; it does not bound what the branch itself wrote about the removal. Round 2 found them by reading, and the three are corrected at `ef5c607d` |
| How the `in one batch` glob decides | Round 1's fix asserted the string's absence in every `agents/*.md` | **the claim, through a question word in the window** | `agent-contract` §10 gives that phrase a second and legitimate meaning — *Batch independent reads and runs* — so a definition writing *open every coordinate in one batch* was following the contract and would have gone red under a message about a question nobody asked. A check that refuses a phrase by spelling rather than by what it claims is the shape this release keeps finding, so it was repaired rather than narrowed by a word. **The first draft of the repair had the same defect one layer down**: a bare `"ask" in window` fires on `task`, and the probe meant to show the legitimate wording passing went red on that word. Matched at a word boundary now, and both directions probed |
| Which modules a narrow run covers | The build and round 1 each chose the set by which files they had edited | **a third module was red from phase 2 and neither run opened it** | `tests/test_review_axes.py::test_implement_and_the_preset_block_do_not_drift` pairs `skills/implement/orchestration.md` with the `CLAUDE.md` block on nine shared phrases. Phase 2 rewrote both and dropped two of them, so the case went red at that commit and stayed red through seven phases, a review round and a fix pass — the build ran the modules its plan rows named, round 1 ran *the six changed modules*, and this one reads two edited files while naming neither. Repaired here: `one \`multiSelect\` question with three checkboxes` became `two questions in ONE \`AskUserQuestion\` call` in the pair, and `What is checked is the answer` was restored to the orchestrator's section, which had lost it |
| How the two documents spell the one-call rule | `orchestration.md` said *Both questions go in one `AskUserQuestion` call*; the block said *Ask it as two questions in ONE `AskUserQuestion` call* | **one spelling, `two questions in ONE `AskUserQuestion` call`** | Two spellings of one decision is exactly the drift the pair above exists to catch, and it is what made the repair visible. Three places now carry the one phrase: both documents and the case that pairs them |
