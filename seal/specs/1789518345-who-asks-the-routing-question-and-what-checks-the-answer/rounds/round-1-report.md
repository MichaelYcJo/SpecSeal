...
`test_a_direct_declaration_with_no_seal_is_a_notice_on_a_draft`. `direct_seal` <!-- NAME NOT IN TREE: round 1 asked for this case to be renamed and the fix pass renamed it to `test_a_direct_declaration_with_no_seal_is_SILENT_on_a_draft`. A record states what the round found, so the old name stands here. -->
