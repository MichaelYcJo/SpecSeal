...
| `tests/test_the_ledger_fragments_fold_at_release.py`'s `tree` fixture puts the version section last; `test_the_section_is_appended_below_the_existing_areas` asserts the last `## ` starts with the version; `test_folding_twice_finds_nothing_and_writes_nothing` re-runs with no new fragment and expects exit 1 | read · NAME NOT IN TREE — work item 1790208593 (#547) re-pointed that case at the release file as `test_the_release_file_is_the_section_and_the_shared_ledger_is_untouched` |
