...
| S15 · the ledger stays true | Rows whose anchors this work moves are re-read and re-stamped with a dated note: `.github/scripts/fold_ledger.py#section@1b72e077`, `#main@2e8fcb28`, `#append@e091419b` (if touched) in §0.4.0's rows; `round_record.py#kept_broad_gate@c2a4e86c` in A9; `tests/test_the_report_standard_is_one_in_three_places.py` anchors if a constant moves. `evidence-check --strict .` exit 0 at each phase close | executed |
