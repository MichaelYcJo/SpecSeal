...
| Fix range | `0c72d95664d158a40c9f2726534bdfa3496af194..44146cf32ab47bb142705c90230395bd5f2fa133`, 2 commits |
...
|---|---|---|---|---|
| 🟡 1 | *for six releases* is an unmeasured count, false in five carriers: seventeen ledger sections and eighteen tags stand between the second `0.9.3` heading and its removal | `.github/scripts/fold_ledger.py:24` (and `tests/test_release_hygiene.py:1120`, `tests/test_the_ledger_fragments_fold_at_release.py:314`, the changelog fragment lines 6–7, fragment row F3's Notes) | **fixed** `bc186868` | fixed at bc186868 — the five carriers state the measured figure: seventeen ledger version sections `0.9.4`–`0.15.0` after the second `0.9.3` heading at 9f846733, eighteen tags after `v0.9.3` (`0.13.2` folded no section), each with how it was counted; executed — `awk 'NR>1673 && /^## [0-9]/'` over the ledger prints 17; `git tag --sort=v:refname` after `v0.9.3` prints 18 |
| ⬜ 2 | the hand-back's *two* doubled marker lines are twenty — 118 marker lines, 98 distinct — and the issue the orchestrator files should carry that number | `seal/specs/1790206437-a-second-fold-writes-a-second-heading/overview.md` §Not done, `phases/phase-5.md` §What this phase found, pull request #552 §Found outside the item | **fixed** `bc186868` | fixed at bc186868 — `overview.md` §Not done and `phases/phase-5.md` say twenty work items (118 marker lines, 98 distinct) and name MichaelYcJo/SpecSeal#553 as the home, taken by step D; executed — `grep -c` and `sort -u \| wc -l` over the marker lines; the twenty pairs' line numbers are in the prose. Paperwork: out of `Needs a fix` |
| ⬜ 3 | two carriers outside phase 3's five phrases still say a held run always stays — the spec's *a re-seal records a second run rather than erasing the first* and the sealer's *never erases the record of the first* | `docs/review-chain-spec.md:342-343`, `agents/sealer.md:140-142` | **fixed** `bc186868` | fixed at bc186868 — and 44146cf3 — the report's two rewordings in `docs/review-chain-spec.md` and `agents/sealer.md`, plus a tenth carrier the sweep over the fix range found, `chain_check.py#broad_gate`'s docstring; two released records of the 0.15.0 wording excused in a new `survivors.md`; `GATE_CARRIERS`' stands phrases intact; read against `same_run` and `templates/sdd-round.md:39`; both rewordings keep the `GATE_CARRIERS` stands phrase. The smith may answer the sealer's with `kept_broad_gate`'s reading; the spec's has none |
| ⬜ 4 | the pull request body carries none of the four gate answers for the new `--check` arm, and `CONTRIBUTING.md` says the prompt budget is answered there or not at all | pull request #552 body | answered | the four gate answers for the `--check` arm belong in the pull request body, which the orchestrator wrote into #552 before round 2; read — `CONTRIBUTING.md:186-190`; the orchestrator writes the body before ready. Out of `Needs a fix` |
