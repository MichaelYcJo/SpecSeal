...
| `tests/test_the_reopening_is_one.py#floor_record`, the one unit under `tests/` reading `no call site found` for a reason this branch does not repair — it is passed by name as a value at five sites and never called, so the reach walk's `name(` never matches | the repository owner. Round 1's finding 5: this was a bullet under an empty table in `seal/follow-up.md`, in the section for items another branch holds, where nothing was waiting on anything. It is tied to a coordinate, so that file's own rule sends it to a `# RIDER:` at the line, which is now planted above `floor_record` and reaches whoever opens the file |
