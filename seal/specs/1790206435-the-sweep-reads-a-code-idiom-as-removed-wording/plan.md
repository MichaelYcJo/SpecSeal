...
| 4 | **The records.** `evidence-check --reverify .` over the rows `spec.md` §*Data & interfaces* names, each re-read first; the ledger fragment's new rows anchored on the phase 1–3 units; `changelog.md`; `overview.md` with its `## Not verified` table; the count of existing `survivors.md` rows of the two classes (Q3) in `overview.md`; the branch's own sweep and its `survivors.md` for the docstring sentences this branch rewrites that ledger rows quote | `evidence-check --strict .` exit 0; the branch's sweep at exit 0 with every report either corrected or under `exempt`; the three narrow modules the edited files are read by | 759af110 |
| 5 | **A file moved and reworded in one commit is read as a deletion plus an addition** (#551, added by the owner after phase 3's Q5 probe). `--no-renames` on both `git diff --name-only` calls (`corrected`, `whole_range`); `corrected`'s docstring says why a pure move is silent now — `wanted` subtracts what is written back — where the silence used to come from rename detection; a case in probe 3's shape (a forty-paragraph file moved with one sentence reworded, git reporting `R09x`, its copy standing in another file) seen red first, and a case that a pure move stays silent with the old path read | The same module; the four real-range cases re-run and any moved count pinned and named in `phases/phase-5.md`; `evidence-check --strict .` exit 0 after `--reverify`; the branch's sweep; the ten record-reading modules | 943651b8 |
...
| round 1 fix, 🟡 1 · round 2 fix, 🟡 1 and 🟡 2 | round 1: the pure-release case at exit 1 naming `docs/a.md` at 2.00 against the section the release moved. Round 2: the reworded release at exit 0, `against 2 sentence(s)`, and the older-release case at exit 0, `against 0 sentence(s)`, both at `1f8cdcda` | The gate **reports less**, and only for a live `CHANGELOG.md` sentence that **this range** wrote, identical, under a version heading — the released sentences at `b` beyond those at `a`. That is a move, not a correction, so the wrong allow is a sentence the range removed from the live section while writing the identical sentence into a release, which is what a release is. A sentence standing in an older release holds nothing, and where the file lost a sentence the released wording is written as any file's is, so a reworded release still splits the removed sentence into its runs. **Corrected 2026-09-24 in round 2's fix pass**: this row first said the held count was every released section at the tip, *counted and never written back*, with the wrong allow *a sentence a release both moved under a heading and removed from a live document in the same commit — which no release commit does*; round 2 executed both halves false. The wrong deny round 1 closed was a regression against the base in the repository shape the heading reading was chosen for | zero | text reads only |
| 5 | the rename case at exit 0 and `against 0 sentence(s)`; the pure-move case at `against 0` | Today the gate is **silent on a rename**: a real survivor of a sentence reworded in a moved file reports nothing, the direction this check exists to stop. After the fix the wrong direction is **noise on a pure move**, and `wanted`'s subtraction of every n-gram written back already answers it — a verbatim move removes sentences and writes each one back | zero | a git flag, `--no-renames`, the same on every platform git runs on |
