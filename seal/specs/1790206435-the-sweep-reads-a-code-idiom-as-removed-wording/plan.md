...
| round 1 fix, 🟡 1 | the release case at exit 1 naming `docs/a.md` at 2.00 against the section the release moved | The gate **reports less**, and only for a sentence standing under a version heading at the tip: a release that moves `## Unreleased` under a version heading no longer counts the section as removed, so a document restating an entry is not reported at the release with nothing to correct. The held count is counted and never written back, so the wrong allow is a sentence a release both moved under a heading and removed from a live document in the same commit — which no release commit does; the wrong deny was a regression against the base in the repository shape the heading reading was chosen for | zero | text reads only |
