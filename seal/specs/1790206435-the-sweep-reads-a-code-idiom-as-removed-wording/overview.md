...
| ✅ the rename shape found under Q5 — a whole file moved as a rename with one sentence reworded is silent — reaches an issue with the paste-ready repair in `phases/phase-3.md` | filed by the orchestrator as #551 and taken here as phase 5 (`phases/phase-5.md`, ledger row R1), 2026-09-24 |

## Not done

- ~~**`--no-renames` on the two `git diff --name-only` calls**, handed back
...
- *inferred during implementation, then built in phase 5*: a verbatim move
  between files used to be silent by git's rename detection before `wanted`
  was reached, and a rename with a small rewording was silent for the same
  reason — judgment 7's mechanism held for a split, where both files
  remain, and not for a rename. With `--no-renames` (#551) it holds for
  both: the `#551` row in `spec.md` §*Scope* is the clause.
