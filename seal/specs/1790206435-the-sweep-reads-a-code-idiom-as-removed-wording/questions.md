...
| Q5 | Does a section moved whole from one file to another — #526's shape, the split of `docs/review-chain-spec.md` — report nothing? `wanted` subtracts every n-gram the range wrote, so a pure move leaves no removed n-gram to look for; a reworded sentence in the move is a source as it should be. Read, not measured, and the milestone's grounds for putting this item first name that split as a range this sweep has to judge cleanly | a measurement — one probe in phase 3, driven from Python: a claim stated in `a.md` and quoted in `notes.md`, moved to `b.md` verbatim in one commit, then moved with one sentence reworded | Silent on the pure move and reporting `notes.md` on the reworded one: recorded in `phases/phase-3.md`, no case planted. Anything else: a finding for the fix pass and a case | the probe's result is recorded either way | ✅ probed 2026-09-24 from Python: the pure move is silent (exit 0) and the reworded one reports `notes.md:3` at 2.00, as judgment 7 said. One thing the judgment did not name: the pure move is silent because git's rename detection drops the old path from `--name-only`, not because `wanted` subtracts it — and a large file moved with one sentence reworded is a rename (`R096`) whose correction the sweep never sees. Outside this item's phases; recorded in `phases/phase-3.md` and handed back with the repair (`--no-renames`) for the orchestrator to file |
