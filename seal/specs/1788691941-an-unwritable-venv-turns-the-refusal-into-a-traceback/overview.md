...
| Whether the existing named case is replaced | `spec.md`'s fourth scenario reads *a case derived from the function's own returnable set*, singular | Both kept | The mutation measured the split: under a sixth value the derived case fails and the named case passes. In the other direction a constant renamed with its value untouched is still the same word in the document, which only the named form catches. Neither covers the other |
