...
- **Failure direction.** Each arm moves toward CI's answer, and that is **not
  ...
  should move the same way — a baseline carried forward drops the rows the
  base's own newer commits added — and that half is **read, not executed**.
  The survivor arm is the half that was measured, in both directions, and the
  repository owner answers whether the baseline half is worth a case of its
  own.
  <!-- CORRECTED 2026-09-21 by work item 1789996775 (#464). What stood here:
  "The two baselines move the same way." It is not false, and that is the
  ...
  `skills/agent-contract/SKILL.md` §4 is the rule against. Round 2's finding
  14 (`rounds/round-2-report.md`:195) names the three sites and its
  §*Paste-ready fixes* carries this replacement; `seal/ledger.md`'s R3 row of
  this work item is where the distinction survived. Corrected in place with
  the issue named, never deleted silently: a record of a past state that
  quietly becomes true is a record nobody can audit. -->
  `spec.md` §*What is wrong* already said so — what a stale
