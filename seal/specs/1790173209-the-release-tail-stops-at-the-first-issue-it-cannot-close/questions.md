...
| Q2 | Which read gives the roll the comment count with the least change to the module's fakes: `gh issue list --json number,title,comments` on the one list call, or `gh api repos/<owner>/<repo>/issues/<n>` through `try_run`? | the work — phase 6, by reading the fakes in `tests/test_a_release_rolls_the_flow_measurement_issue.py` | Either is one read and no write. The fakes match on argv prefix, so the choice is whichever prefix they already answer. | the list call's `--json` | ✅ answered by phase 6: the list call asks `number,title,comments`, because every fake that reaches `main` already answers `list_open_issues` and a second read would have sent one case to the network (`phases/phase-6.md`) |
