...
| 3 | The span's own disclosure in `skills/verify/SKILL.md`, the two fragments, and every ledger row the edits drift — **fourteen** rows over seven anchors, not the six the handoff named, because `#report` and the skill's own section drift too | the modules the skill change touches; `evidence-check`; `survivor-check` over the whole fix range | `80cc7ce` · `62347dc` |
