...
| An `&` anywhere but at the end | **Inferred during implementation, and added by round 2's ⬜ 3.** This list originally held three forms and the refused list held a trailing `&`, so a mid-line `&` was in neither — round 1's 🟡 1. It breaks the criterion's second half wherever it stands, so it had to be in one of them, and it is here because telling an operator `&` from a `2>&1` or a quoted one needs the shell parser the row above refuses. Its cost is named in `templates/config.md`: the command before it is backgrounded and may still be running when the gate stamps, under `/bin/sh`. Whether to refuse it in code instead is `questions.md` Q6, which a fix pass may not decide |
