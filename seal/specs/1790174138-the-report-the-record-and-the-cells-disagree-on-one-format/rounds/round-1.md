...
|---|---|---|---|---|
| 🟡 1 | `close --broad-gate` is the second writer of the `Broad gate` cell and still replaces a run the cell already holds; three documents say it writes "the same cell" | `skills/code-review/scripts/round_record.py:3883`, `:4067`, `templates/sdd-round.md:39`, `skills/code-review/orchestration.md:535` | **fixed** `5bbb19e5` | fixed at 5bbb19e5 — `kept_broad_gate` is the one cell-writing path for `seal` and `close --broad-gate`; the `close` case seen red at HEAD (the cell replaced by one entry) then green; `templates/sdd-round.md`, `skills/code-review/orchestration.md` §READY and `chain_check.broad_gate`'s message say the same path. ⬜: a re-seal at the commit the newest entry already names replaces that entry rather than duplicating it (prefix comparison, no git call), case seen red first; `bound_line` reports a running walk that already reached two in the gate's own refusal sentence and the unreachable `reaches 3 here` branch is removed, the counted-two case re-pinned; read — `gate = cell(BROAD_GATE, args.broad_gate)` is built from the flag alone and written over the row; `seal` keeps the held run at `:4441-4444`; reachable through the header-only fix table `seal`'s docstring names and through a partial table, since `Pass` ticks only when no row is open |
| 🟡 2 | a ninth carrier of the one-run wording stands in the reviewer's own file, and no test pins any of the eight the branch rewrote | `agents/warden.md:320-322`; `plan.md:74-77` | **fixed** `915a7b2c` | fixed at 915a7b2c — the ninth carrier in `agents/warden.md` §Role rewritten; new module `tests/test_the_broad_gate_cell_keeps_every_run.py` pins the eight rewritten documents as gone/stands pairs, seen red by restoring `skills/verify/SKILL.md`'s old sentence; the ninth carrier is the warden's own §Role sentence, rewritten in that file, and the comment `new_broad_gate_file` writes into `broad-gate.md` is a code comment no test pins (**Corrected 2026-09-24** by round 2's paperwork row: the cell said nine documents were pinned). ⬜: the warden's "next `##`" sentence now says own level or shallower; the unsourced "71 work items" aggregate dropped from the spec and the review skill with #437 as the source; the three-places module's docstring states its count of four; executed — `grep -rn "the SHA it ran at"` over `agents docs skills templates` hits this line alone outside the rewritten files; `grep -rn "newest first\|earlier run" tests/` hits one docstring; read — the plan says a pin makes the class red and A16 pins something else |
| 🟡 3 | a second table's header row under a `###` inside `## Verdicts` is copied into the record as a verdict row at exit 0, and the checker reads the same shape on a record | `skills/code-review/scripts/round_record.py:1086-1090`, `skills/code-review/scripts/chain_check.py:1569-1583` | **fixed** `5bbb19e5` | fixed at 5bbb19e5 — `table_body` and `chain_check.verdict_table` both skip a row that repeats the header, like a separator; the generator case (second table under a `###` inside `## Verdicts`) and the checker case each seen red at HEAD then green; the second table's real rows are carried. `21b8d61e` carries ledger rows A9–A11 and 29 re-read rows. The orchestrator re-ran at `21b8d61e`: seven modules passed, tree-wide ruff clean, `evidence-check --strict` exit 0, CI-form sweep one excused; executed — the probe in §Executed probes; read — `table_body` drops separators only and `finding_number` admits `#` as a no-id cell; `verdict_table` skips separators only |
| ⬜ 4 | "a section ends at the next `##`" names one of the two enders | `agents/warden.md:466` | **fixed** `915a7b2c` | fixed at 915a7b2c — the warden's section-end sentence names both enders: own level or shallower, a `##` or a `#`, never a `###`; read — `section_end` ends at own level or shallower, so a `#` ends it too |
| ⬜ 5 | a running walk that already reached two prints "reaches N+1 here" where the gate already refuses at the floor record with N — #218's sibling | `skills/code-review/scripts/round_record.py:2093-2102` | **fixed** `5bbb19e5` | fixed at 5bbb19e5 — `bound_line` reports a running walk that already reached two in the gate's refusal sentence; the unreachable `reaches 3 here` branch removed; `test_the_count_walks_message_says_records_when_it_counted_two` re-pinned to that sentence, seen red first; read — `fires` at `:2013` admits a running walk of two; the stopped branch at `:2081` is the sentence for it and the running branch falls past it; verdict unchanged |
| ⬜ 6 | "Measured over the 71 work items …" states an aggregate without source or tree state; 25 records exist at the target SHA | `docs/review-chain-spec.md:982-984`, `skills/code-review/SKILL.md:332-334` | **fixed** `915a7b2c` | fixed at 915a7b2c — the "71 work items" aggregate dropped from `docs/review-chain-spec.md` and `skills/code-review/SKILL.md`, #437 named as the source; read — the number is #437's; executed — 25 committed round records at `e3cc83c9` |
| ⬜ 7 | the module named for three places pins four carriers | `tests/test_the_report_standard_is_one_in_three_places.py` | **fixed** `915a7b2c` | fixed at 915a7b2c — the three-places module's docstring states what it counts (the three the reviewer copies, and the template as a fourth); the file name kept so the fragment's A6/A7 anchors and the phase records still resolve; read — `CARRIERS` has four entries |
| ⬜ 8 | a re-seal at the commit the cell already names appends a duplicate entry; none of `seal`'s refusals is *already holds this run* | `skills/code-review/scripts/round_record.py:4441-4444` | **fixed** `5bbb19e5` | fixed at 5bbb19e5 — a re-seal at the commit the newest entry already names replaces that entry (prefix comparison, no git call), `test_a_re_seal_at_the_commit_the_cell_names_replaces_that_entry` seen red at HEAD with three entries then green; recorded in `kept_broad_gate`'s docstring and the template comment; read — the keep is keyed on a held SHA, not on a different one; a question for the owner, not a defect by the cell's definition |
