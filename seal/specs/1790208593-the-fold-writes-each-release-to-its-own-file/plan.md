...
Six phases, in the order the risk is retired (five framed; #553's joined at
phase 3 on the coordinator's instruction during phase 2). Phase 1 widens every reader
...
fold is what ties the two phases together. Phase 3 is #553: a fragment's own
marker line is folded once, `--check` refuses a doubled marker, and the
shared ledger's twenty doubled markers go. Phase 4 is `--split`, with its
anchor rewrite and a rehearsal over a copy of this repository's ledger.
Phase 5 is the documents and the ledger's header. Phase 6 verifies: the
...
and phase 4's rehearsal needs C's `0.9.3` repair (`d08c671a`) in the copied
