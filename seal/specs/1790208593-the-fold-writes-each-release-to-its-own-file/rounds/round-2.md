...
|---|---|---|---|---|
| 🟡 1 | `CLAUDE.md`'s removal rule and its instead-of table name `seal/ledger.md` alone, while the twin in `CONTRIBUTING.md` names the release files too; *`CONTRIBUTING.md` carries the same sentence* is false | `CLAUDE.md:139` (and `:134`) | **fixed** `94e6d6cd` | fixed at 94e6d6cd — by the orchestrator, under the owner's `CLAUDE.md` authorization: the instead-of table and the removal rule name `seal/releases/<X.Y.Z>.md` beside `seal/ledger.md`, matching `CONTRIBUTING.md`; row C8 re-stamped; read — against `CONTRIBUTING.md:198` and `:210`; phase 5 enumerated two paragraphs and not this one; the answerer is the orchestrator under the owner's authorization for `47ca9ce6`, or the owner |
| 🟡 2 | The release checklist says `evidence_check.py --strict .` reports a count of distinct (status, coordinate) pairs; nothing prints that count | `docs/release-checklist.md:152` | **fixed** `3f5e9d75` | fixed at 3f5e9d75 — the checklist compares `evidence_check.py --strict .` at exit 0 with 0 drifted and 0 broken, and the count of ledger table lines before (`cat seal/ledger.md seal/ledger/*.md \| grep -c '^\|'`) and after the split (`… seal/releases/*.md …`), 1027 both times on a scratch copy; fragment row P2 records that the distinct count is a module-level measurement no command prints; executed — the checker's output at the target is per-file and total ok/drifted/broken only; the count needed an in-process probe; the table-line count by `grep -c` reads 1027 before and after |
| ⬜ 3 | The split rewrites a line anchor whose line the standing area also holds, which turns a row the checker resolved by hash to the kept copy into DRIFTED; a line that two moved sections share and the standing area holds is kept in silence | `.github/scripts/fold_ledger.py:579` | **fixed** `b1af1403` | fixed at b1af1403 — a line that stands in more than one place is named for a person rather than moved or kept, since only the row's hash can choose; the reviewer's case and a second shape, both red at 94e6d6cd; the real-tree dry run byte-identical; executed — probe: `6 ok · 0 drifted` before, the anchor listed as a rewrite, `5 ok · 1 drifted` exit 1 after; the fix names it, and the real-tree dry run is unchanged; not on the real ledger |
| ⬜ 4 | The escaped-quote symptom of round 1's first finding is fixed and pinned by no case | `tests/test_the_ledger_fragments_fold_at_release.py:842` | **fixed** `b1af1403` | fixed at b1af1403 — `test_the_split_reads_an_escaped_quote_the_way_the_checker_does`, red with the `\\"` branch removed; executed — a mutant dropping the `\\"` alternative passes the fold module (62 passed); the proposed case is red on the mutant and on the pre-fix script, and green at the target |
| ⬜ 5 | The conflict rule's heading names `seal/ledger.md` and the release files but not a fragment, and this branch's own merge conflicted in one; the account said it names every ledger file | `CLAUDE.md:146` (and `CONTRIBUTING.md:225`) | **fixed** `94e6d6cd` | fixed at 94e6d6cd — and 3f5e9d75 — the conflict heading in `CLAUDE.md` (the orchestrator's) and `CONTRIBUTING.md` names a fragment two stacked branches both edited; `test_a8_both_rule_documents_say_what_to_do_at_the_conflict` green; read — `dff99894`'s conflicts in `seal/ledger/1790206437-a-second-fold-writes-a-second-heading.md`; the answerer is the owner |
| ⬜ 6 | Two test docstrings still say the fold writes into the shared file, in the present tense | `tests/test_a_merge_cannot_silently_drop_a_correction.py:1135` (and `tests/test_the_record_is_held_to_the_floor_and_the_depth.py:504`) | **fixed** `3f5e9d75` | fixed at 3f5e9d75 — the two test docstrings in the past tense or naming the release files; read — the twins of round 1's eighth finding and of `1d0c61b8`'s survivor, one in the same file as that survivor |
| ⬜ 7 | Three edited lines are left unwrapped, at 110–146 characters | `.github/workflows/hygiene.yml:123` (and `CLAUDE.md:147`, `skills/evidence-check/scripts/correction_check.py:238`) | **fixed** `94e6d6cd` | fixed at 94e6d6cd — and 3f5e9d75 — the three long lines wrapped; read — no wrap case covers these files, so nothing reports them |
| ⬜ 8 | This repository's `seal/README.md` layout, verbatim from the template, leaves out `releases/`, which holds most of the rows after the split | `templates/seal-README.md:79` | answered | the shipped template names no repository's release files by design: the owner chose a template true for every repository, and this repository's `seal/releases/` layout is stated in `CLAUDE.md` and `docs/the-evidence-ledger.md`; read — the template already lists optional entries (`parity.md`, `follow-up.md`); a question for the owner |
