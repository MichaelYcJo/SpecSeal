...
| **#553, joined during the build**: the fold drops a fragment's own leading `<!-- specs/<id> -->` line before writing its own, so a folded section carries one marker; `--check` refuses a marker standing twice in `seal/ledger.md` or a release file, naming the file and both lines; the doubled markers in this repository's `seal/ledger.md` are removed with their blank lines, every row byte-identical — twenty work items measured at `9f846733` (`grep '^<!-- specs/' \| sort \| uniq -d`), where the ticket counted two | Phase 3 |
| The documents that name the address, and the ledger's own header | Phase 5 |
| The fragment rows, the changelog entry stating what the split buys and does not, the memo, the sweep | Phase 6 |
...
| One row anchors into `seal/ledger.md` itself: the row at line 2521 (§0.13.1, `1790076070`) cites `seal/ledger.md#"### 1788331011-two-roots-hold-three-lifetimes"` and `seal/ledger.md#"### 1788398967-local-modes-records-never-leave-the-clone"`, both headings in §0.4.0 and §0.5.0. No fragment, document, skill, test or hook cites `seal/ledger.md#` | executed — `grep -rn 'seal/ledger\.md#'` over the ledger, `seal/ledger/`, `docs/`, `skills/`, `agents/`, `tests/`, `hooks/`, `.github/` |
...
| S21 · a fragment's own marker is folded once (#553) | Given the fold fixture with a fragment whose first line is its own `<!-- specs/<id> -->`. When the fold runs, then the release file carries that marker on exactly one line and `--check` counts the work item once; given a ledger or release file where a marker stands twice, `--check` exits 1 naming the file and both lines; this repository's `seal/ledger.md` carries no doubled marker, and the hygiene module refuses one on every pull request | red first on the fixture (the marker stood twice) and on a copy of the ledger as it stands (twenty pairs) |
