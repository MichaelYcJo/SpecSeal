# 1790297085-settle-retires-a-directory-main-has-not-seen-closed — routing
...
| Branch | fix/602-settle-retires-a-directory-main-has-not-seen-closed |
...
#602, #590 and #586, `settle` and the release scripts: `--retire` before the closure reaches the base, the exit code for a missing sibling, and a fragment carrying its own `## ` line. The owner pressed `automation` once for every work item of `release: 0.15.4` in one question.
