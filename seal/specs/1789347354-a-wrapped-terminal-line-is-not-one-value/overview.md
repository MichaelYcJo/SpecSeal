...
· verified: executed — the four modules below, the record module before and after; every alternative of both constants mutated in turn — killed outright, and, where it has one, with its lookahead dropped — and every sentence this work pins stashed in turn — the results are per-alternative in ledger rows 1 and 2 rather than as a total here, because a count is the third thing this work item got wrong about its own measurements, `bin/evidence-check .`, `--strict`, and the Q5 measurement over 337 terminal rows. Read — the frame, the round-4 report, `seal/ledger.md` R7 and the `issue_claims_check.py` rows. Unverified — the full suite, repository-wide lint and typecheck, which are the sealer's
...
| Three of `survivor_check.py#BLOCK`'s five alternatives are unpinned against being killed outright — `[-*+]`, `\#{1,6}` and `>` — and the ordered-list alternative is unpinned against losing its lookahead. Round 2's 🟡 1 took the ordered-list one as the alternative whose loss nothing catches. **Round 3 measured that killing `[-*+]` or `\#{1,6}` outright turns nothing red either**, which is the opposite of what this row and the case's comment said: what the existing case catches for those two is the lookahead being dropped, a widening, and not the killing, a narrowing. Filed as #393 with the two arms that close the ordered-list half | the repository owner, as a ticket against `survivor_check.py` — round 3 is this work item's last round, so a case written there would ship unread |
