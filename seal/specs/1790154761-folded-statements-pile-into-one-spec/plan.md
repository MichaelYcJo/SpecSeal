...
| 4 | **The work item closes.** `changelog.md` fragment, `seal/ledger/1790154761-folded-statements-pile-into-one-spec.md` rows (the three test modules' units, settle §2, the evidence-ledger paragraph, the Korean section), and `overview.md` with `## Not verified` (the broad gate, answered by the sealer) | `evidence-check --strict .` executed on the fragment, and `unverified-check` executed on the overview || aebb98ae |
