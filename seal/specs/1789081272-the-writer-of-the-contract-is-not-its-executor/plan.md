...
|---|---|---|---|
| 1 | `agents/framer.md` — contract opening verbatim, `## What you are`, the three writes named under §6, what it reads, the report shape. `tests/test_docs_line_wrap.py` `COVERED` gains it | `test_every_agent_reads_the_contract` and `test_a_moved_rule_leaves_its_definition` at five files · `test_only_one_definition_assigns_the_broad_gate` still `["sealer.md"]` · `test_docs_line_wrap` on the new path · each shown red first per §15 | `77e0ae5` |
