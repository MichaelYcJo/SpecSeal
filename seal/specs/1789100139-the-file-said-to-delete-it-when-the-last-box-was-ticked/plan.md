...
|---|---|---|---|
| 1 | `## Order inside a ticket` lands in `skills/implement/orchestration.md` as an `Orchestrator:`-prefixed section; `tests/test_the_rules_have_one_owner.py`'s `FLOW` and its two cases move to it | the two cases seen red against the pre-move text, green after; `bin/test -q tests/test_the_rules_have_one_owner.py` | |
