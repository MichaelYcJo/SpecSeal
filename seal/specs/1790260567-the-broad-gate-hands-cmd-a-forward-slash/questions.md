...
| Q2 | With `GH_CONFIG_DIR` set to an empty directory and the four token variables unset, does a `gh` that is authenticated through the OS keyring report "not logged in"? Reading cannot answer this, because whether `gh` reaches the keyring without a `hosts.yml` depends on the `gh` version. | a measurement: one probe in phase 3 on the builder's machine, before the conftest block is written. `GH_CONFIG_DIR=<empty dir> gh auth status; echo $?`, read directly (contract §1) | **Yes:** the block is those five lines. **No:** also set `GH_HOST` to a neutral host (`example.com`, per the repository's no-real-identifiers rule) that no keyring holds a token for, and record the probe in `phases/phase-3.md` | The block as `spec.md` Scope 3 states it, with the fallback held ready | ✅ measured 2026-09-25 at `6cc67d0b`, before the block was written, on the builder's machine with `gh` 2.100.0 logged in through `hosts.yml`: `gh auth status` exited 0 as it stood, and exited 1 with an empty `GH_CONFIG_DIR` and no token variable, "You are not logged into any GitHub hosts". **Corrected 2026-09-25** in round 1's fix pass: that probe measured a login kept in `hosts.yml`, not one kept in the OS keyring, so it did not answer the question as asked. Read from the upstream `cli/cli` repository's `internal/config/config.go`: with no token variable, `ActiveToken` falls back to the keyring's active-account slot even where no `hosts.yml` exists, so an empty `GH_CONFIG_DIR` does not hide a keyring login (round 1's 🟡 1). The keyring path is now closed by the env token: the conftest sets `GH_TOKEN` and `GH_ENTERPRISE_TOKEN` to a placeholder, which `ActiveToken` takes before it reaches the keyring. Executed here: `gh auth token` under the suite's environment returns the placeholder, and `gh api user` fails `Bad credentials (HTTP 401)`. The keyring half itself is `read`: this machine keeps no keyring login. The fallback host variable was not written, and would not have been enough (`-R owner/repo` resolves its host from the repository) · NAME NOT IN TREE |
| Q3 | Does any case start a child process whose environment is built from nothing, so that it inherits neither the scrub nor `os.environ`? A static read on 2026-09-24 found only in-process `env=` parameters (`test_a_body_naming_two_issues_claims_one.py`'s `main(env={})`, `test_lint_python.py`'s `_ran(…, env=…)`), and every child `env=` it saw spreads `os.environ`. That read was not exhaustive over `Popen` or wrapper helpers. | the work: phase 3 greps every `subprocess`/`Popen` call in `tests/` with an `env=` argument that is not derived from `os.environ` | **None found:** the enumeration is recorded and nothing changes. **Some found:** each one either spreads `os.environ` or carries the scrub's variables, fixed in phase 3 and named in `phases/phase-3.md` | None escape | ✅ read 2026-09-25: every `env=` built in `tests/` spreads `os.environ` or is handed to an in-process `main`, and the two `Popen` calls pass no `env=` (`phases/phase-3.md` lists each). None escape, and nothing changed |
