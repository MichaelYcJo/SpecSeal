...
| 2 | `open_issue` writes a body linking the closed issue and the durable ledger | `test_one_open_issue_closes_it_and_opens_the_next` extended, seen red before the code | 8e87f94 |
| 3 | The same call adds `measurement` and `log: measurement`, best-effort, and a failure to set either is written into the created issue's body | a case with the milestone call failing: the issue is still created, the body says so, the run does not fail | 463f41d |
| 4 | This repository's `#51` gets `flow-baseline`; the fragments | `evidence-check` on the new rows, `fold_ledger.py --version 0.8.0 --dry-run` | 5e76246 |
