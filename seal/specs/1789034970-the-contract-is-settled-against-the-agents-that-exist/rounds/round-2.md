...
| 8 | The count case's first failure message points at §2 when two literals in the case are what moved | `tests/test_broad_gate_rule.py:281` <!-- Read 2026-09-17 by 1789621028-nothing-reads-a-record-against-the-tree, and the position is WRONG and was wrong when written. `:281` holds the module constant `COUNT_WORD`; the failure message this row is about begins at `:328`, *§2 no longer states the count this case checks, so the number and the sentence have come apart*. At `b8aa637`, the commit that wrote this row, `:281` already held `COUNT_WORD` and `:315` — the line #344 names — held `for path in agents`. The cell is NOT re-pointed: a `Location` is a position, positions rot, and turning them into content anchors is `questions.md` Q1 of that work item, refused there as its own work item across 227 records. This comment is the correction a refused migration allows. --> | **fixed** `60c352e` | fixed at 60c352e — ``; Rewording §2's sentence reports *the number and the sentence have come apart*. At 0.11.0 the framer moves §2's prose, `COUNT_WORD` and the `["sealer.md"]` comparison together, and the first assertion masks the other two |
