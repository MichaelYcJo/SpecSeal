...
| 3 | `chain_check.py`: `deferred <home>` closes, bare `deferred` stays open; the reopening bound behind `REOPEN_FROM`, refusing the second fix-closing record after a floor record and printing for earlier items; `docs/review-chain-spec.md` gains the subsection beside the eight | each refusal seen red at a named fixture; the two-record run kept green; the cutoff case at the boundary second; this repository's own records under the new arm | `d6b4fed` |
| 4a | The rule owners — `skills/code-review/SKILL.md`, `docs/review-chain-spec.md`, `templates/sdd-round.md`: the generator performs the four reach-backs (the sections that told the orchestrator to write them now name `round_record.py new` and `close`); the reopening is one, with the *Unless* paragraphs and the *nothing here runs away* paragraphs re-pointed at the spec's subsection; a fix pass adds no mechanism (the first level, before the depth); 🟡 means a defect the release would ship and ⬜ exists; the draft pull request before round 1; a compacted session hands over; the orchestrator re-runs a closed phase's suite before the next (rule 9); the moratorium under *What the record carries*; the template says a record is generated | every changed sentence pinned, each pin seen red with the sentence stashed; the count-rule carrier pin re-pointed; every suite that reads the three files | `8c9c245` |
| 4b | The linking carriers — `docs/review-handoff-protocol.md` (§round-N.md says the record is generated; a new subsection after §While the implementer runs for rule 9), `agents/warden.md` (§Role: a finding whose Location is a record is a correction and not counted by `Needs a fix`; §Report: per finding, *would the release ship a defect* decides 🟡 against ⬜), `agents/smith.md` and `skills/implement/SKILL.md` §5 (link to the no-mechanism rule), `docs/flow.md` §Order inside a ticket (the draft pull request opens at the end of the build); a sweep for every remaining carrier of the count rule and of *the orchestrator writes the record* | every changed sentence pinned and seen red; every suite that reads the five files; `grep` for the count rule's phrases returns the owner and links only | `12c6db6` |
| 5 | The fragments and the memo — `changelog.md`, `seal/ledger/<id>.md` rows for the generator, the bound and the vocabulary; `overview.md` closed; the draft pull request opened | `evidence-check .` unscoped, `fold_ledger --check`, `unverified-check`, `chain_check --worktree` on this item | `8befff2` |

Phase 1 before everything: this branch's round 1 record is the generator's first fixture (A3), so the writer exists before the first round. Phase 3's subsection in the same phase as its refusal, the ordering this repository has learned four times. Phases 4a and 4b last among the rule edits because their pins are what the earlier phases' tests would otherwise be written against twice; split by file group on 2026-09-05 so neither segment runs long enough to compact (#161 term 4).
