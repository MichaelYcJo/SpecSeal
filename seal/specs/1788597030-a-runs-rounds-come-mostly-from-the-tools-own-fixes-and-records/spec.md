...
Where each rule landed, as built by phases 4a and 4b (`tests/test_the_rules_have_one_owner.py` holds the same table as pins): 1 — owner `docs/review-chain-spec.md` §*The last round verifies*, links in `skills/code-review/SKILL.md` and `agents/warden.md` §Role · 2 — owner `skills/code-review/SKILL.md` §*A fix pass adds the unit that pins it*, links in `agents/smith.md` and `skills/implement/SKILL.md` §5 · 3 — owner the skill's §*Findings format*, link in `agents/warden.md` §Report · 4 — owner the spec's §*The reopening — one, and then the run is capped*, links in the skill, `templates/sdd-round.md`, `docs/review-handoff-protocol.md` §*Loses a record or crashes* and `agents/warden.md` §Role · 5 — owner `agents/smith.md`, link in the skill · 6 — owner the skill's §*Orchestrator: the pull request opens before round 1, and a phase is re-run*, links in the protocol's §*The handoff before round 1* and `docs/flow.md` §*Order inside a ticket* · 7 and 8 — owners only, the skill and the spec's §*What the record carries* · 9 — owner the same skill section as 6, link in the protocol's §*After a phase — the hand-back's claim is re-run*.
