# 1790297087-a-ledger-row-that-will-not-parse-is-counted — routing
...
| Branch | fix/299-a-ledger-row-that-will-not-parse-is-counted |
...
#299 and #322, the ledger checker's silences: a row whose hash does not parse is dropped from a clean total, and an invalid escape that a future Python refuses. The owner pressed `automation` once for every work item of `release: 0.15.4` in one question.
