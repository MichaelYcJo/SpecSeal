...
| Rule (a)'s reach | `spec.md` §*In* item 1 (a): *still holds a `#` or `@` after every `ANCHOR_RE` and `OLD_COORD_RE` match in it is blanked* | Leftover code spans holding a `#` or `@`, and words outside a span holding both | An issue number beside a good anchor, `` `a.py#f@…` (#299) ``, holds a `#` after blanking. Read literally, the rule refuses that cell and an installing repository goes red on prose. `spec.md`'s own wording of the class is *a coordinate somebody wrote that the pattern refused*, and every shape it lists has both marks or sits in a span. Pinned in S7's ledger; mutation shows each half held by a case |
