...
- A released work item that wrote no `spec.md` is retired by a rule now,
  ...
  row printed; closing the row or re-homing it is what lets the next
  retirement take it. Before, these directories had no way out, and after
...
- The three pull-request checks read a rule retirement as one, through the
  ...
  refuses a directory that held a `spec.md` or an open row at the merge
  base, and a file removed from a directory that stays.
...
- `settle --retire` no longer removes a directory a ledger row anchors
  ...
  prose is written, including rows above the ledger's first section marker.
