...
| 🔴 1 | `dc2a752` re-anchored the Comparison-axes ledger row to a hash computed on the wrong side of its own edit — DRIFTED at HEAD, and CI's `evidence_check.py .` (no `--strict`, test.yml:87) exits 1 on it | `.specseal/map/1788272986-the-fixes-are-what-open-the-next-round.md` row `skills/code-review/SKILL.md#"## Comparison axes"@3da5dc89` | fixed — round-3 reproduced 67 ok · 0 drifted at d3aa145 | executed: `evidence_check.py --strict .` → 66 ok · 1 drifted, "content changed at 44-123 — re-verify". The section's content is correct; the hash is what is stale. Fix: `--reverify` the fragment |
