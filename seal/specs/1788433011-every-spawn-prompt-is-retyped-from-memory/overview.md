...
| `docs/flow.md`'s roadmap restructuring rode this branch | none of `spec.md`'s Scope table, `plan.md`'s six phases, or this file names `docs/flow.md` | three commits (`96aa3d2`, `e0d3d27`, `28a1400`) move issues between milestones and add a milestone carrying #30, #84 and #120 | orchestrator bookkeeping, not a phase deliverable — decided in conversation with the repository owner as GitHub milestones were restructured. The release sequence changed as a direct consequence of what phases 1–4 measured (the phase-2 CI-lint miss at `A file outside phase 4's scope` above, and the phase-split cost data this branch produced), so the restructuring was decided in the same window this branch was open rather than deferred to a separate item. No phase's Verified-by column covers it and none should: it changes no test, no code path, and no agent's behavior |
