...
| `seal/specs/1789356180-the-two-halves-of-one-generator-refuse-each-other/phases/phase-1.md` | `The last row is the one that matters: the frame's 103 is low by more than half, and phase 3's acceptance re-measures it.` | Phase 1's record of what phase 1 measured, and still exactly true — the count is 210 against the frame's 103. The overview's version of the sentence was rewritten because its ROW changed, not because the fact did. The sentence that FOLLOWED it was not noise and is not exempted: *one row in thirty-seven is still one row in thirty-seven* was overturned by 🟡 2 and carries a dated correction note instead |
| `seal/specs/1789356180-the-two-halves-of-one-generator-refuse-each-other/plan.md` | `That is the one direction this change fails in, and it is why phase 1 carries the instruction into `agents/warden.md`, `templates/sdd-round.md` and` | The approved plan's own text, deliberately left standing with a dated correction note under it — rewriting it would falsify the record of what was approved on 2026-09-14. The note says what round 1 found: the direction is one step further than this paragraph states, and what the repair leaves unclosed. The check sees the standing sentence because a note does not remove it, which is the intended state |
