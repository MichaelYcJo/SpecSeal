...
| six mutations, one per arm | ~~4 red, 2 green — `spec.loader is None` and `SyntaxError`~~ **corrected 2026-09-08 by work item 1788844300 at `b8471fb`: 3 red, 3 green. `ImportError` was green too — re-executed against this module as it stood at `c3c22c2`, deleting `ImportError` from the except tuple leaves 27 passed, exit 0, the same reading `SyntaxError` gives. The arm went uncounted because the except tuple has THREE members and was read as two, which is also why the count below says four arms where the function has five** |
