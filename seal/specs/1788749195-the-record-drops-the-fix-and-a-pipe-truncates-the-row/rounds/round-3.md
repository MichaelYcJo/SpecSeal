...
|---|---|---|---|---|
| 1 | 🟡 `reader()` has four failure paths and the enumeration pinned two; `SyntaxError` is a live arm no case watches, and R5 wrote the count down as two | `hooks/review-history-guard.py:101` | deferred #209 | #209 |
...
| 5 | ⬜ round 2's finding 4 grounds claim an assertion of `chain_module`'s absence that does not exist | `seal/specs/1788749195-the-record-drops-the-fix-and-a-pipe-truncates-the-row/rounds/round-2.md:62` | answered | corrected at `b73bdd3` — `rounds/round-2.md`'s grounds cell no longer claims an assertion nothing makes, and says what is true instead |
...
| 10 | ✅ round 2's ⬜ 4 — `chain_module` a one-line alias | `tests/test_the_record_is_generated.py:64` | answered | executed: the name is gone, `check_module()` has nine call sites, 147 passed |
...
| six mutations, one per arm | 4 red, 2 green — `spec.loader is None` and `SyntaxError` |
