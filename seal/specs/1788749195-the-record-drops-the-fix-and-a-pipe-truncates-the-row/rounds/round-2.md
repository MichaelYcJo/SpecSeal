...
| 4 | ⬜ `chain_module` is a one-line alias for `check_module`, one call site against eight | `tests/test_the_record_is_generated.py:64` | **fixed** `c19766f` | fixed at c19766f — `` — `chain_module` deleted and its one call site moved to `check_module` — 9 call sites there now. **Corrected by round 3, finding 5**: this cell also claimed the absence was asserted, and nothing asserts it; the name is simply gone from the tree; read |
