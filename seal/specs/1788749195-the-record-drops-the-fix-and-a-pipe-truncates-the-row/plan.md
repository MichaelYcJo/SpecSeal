...
|---|---|---|---|
| 1 | A bare `\|` inside a copied cell is escaped as the row is written, in every table `new` copies and in `close`'s fix table. The record's row count matches its header | Cases over fixture reports for each table, each seen red first; the module's existing cases | |
