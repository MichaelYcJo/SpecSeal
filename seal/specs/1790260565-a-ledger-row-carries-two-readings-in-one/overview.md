...
| The corpus case's width | Plan: "The corpus case supplies the ledger-row width read from `templates/ledger.md`" | the corpus case and the unit case both call one helper, `ledger_overwide`, which supplies it | Mutation: with the width argument removed from the corpus case alone, it stayed green, because no row in the tree is overwide. Through the helper, the unit case goes red when the width is dropped inside `ledger_overwide`. When the corpus case goes back to calling `overwide_rows` bare, no case goes red (round 1's ⬜ 2) |
