...
|---|---|---|---|
| How far run 6's result reaches | The ticket: *"So `/reload-plugins` is what refreshes what an agent will be handed, and a full restart is not required to get it."* The handoff narrowed it to skill bodies for spawned agents and named hooks and agent definitions as the unmeasured half | **Three unmeasured things, not two.** Hooks, agent definitions, **and** moving a running session onto a newly installed version | Run 6's sentinel sat in the running version's own cache directory, so what it measured is a re-read of the copy already in force. The experiment record carries this without the version number: §*Method* installs no second version — it edits a skill in place, spawns, reads the answer and restores from a backup — and §*What it established* item 2 names the version cache as *the copy that loads*, with runs 1–4 recorded as failures **because** they edited the marketplace clone instead. The path with the version in it is at `seal/specs/1788433011-every-spawn-prompt-is-retyped-from-memory/questions.md` Q1, which names `~/.claude/plugins/cache/specseal/specseal/0.5.0/skills/writing-style/SKILL.md` as the copy the sentinel was planted in. A user reading this notice is in the other case — a *new* directory exists — and nothing has measured that. `implement` §1 ranks a ticket above the code and below what was ratified; the experiment record is the ratified half |
| How much of the boundary the banner carries | `plan.md` §Alternatives, chosen: *"The notice carries the short form and the skill carries the boundary in full."* The notice as built carried all three axes, both halves of the gap and the evidence label — 707 characters, its fourth line 347 | **The plan's short form. The code moved, not the plan.** 620 characters, fourth line 260 | Round 1, finding 3, measured by calling `notice((0,7,1),(0,8,0))`. The shorter form carries every claim finding 1 needs and drops nothing a user acts on, so there was no case for rewriting the plan to match what shipped. What the skill carries that the notice does not is the settling method and the table of evidence labels — which is what *the boundary in full* meant |
