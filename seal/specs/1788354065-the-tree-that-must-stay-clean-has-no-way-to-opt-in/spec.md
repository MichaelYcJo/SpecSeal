...
| S8 | **Switching modes is documented, not automated** | Given either mode, when the person wants the other, then the READMEs' *Shared or local* section gives the move, from the repository root: local → shared is `mv "$(git rev-parse --git-common-dir)/seal" "$(git rev-parse --show-toplevel)/seal"`, `git add "$(git rev-parse --show-toplevel)/seal"`, commit; shared → local is `git rm -r --cached "$(git rev-parse --show-toplevel)/seal"`, `mv "$(git rev-parse --show-toplevel)/seal" "$(git rev-parse --git-common-dir)/seal"`, commit the removal — *inferred during implementation* (round 1, 🟡 3): spelled relative to the working directory, the moves run from a subdirectory created and staged `src/seal/` and opted the repository out — and the hygiene workflow is installed or removed by hand, from `$CLAUDE_PLUGIN_ROOT/templates/hygiene.yml`. The hooks need no restart: the next command reads the folder where it is. Export/import is #81 and is named as such | `tests/test_first_setup_asks_once.py`: both READMEs carry the section with both moves and "#81", and the block read out of each README and run from a subdirectory lands the root at `<repo>/seal/` one way and back under the common directory the other; `tests/test_docs_line_wrap.py` on the READMEs; the two READMEs in one commit |
