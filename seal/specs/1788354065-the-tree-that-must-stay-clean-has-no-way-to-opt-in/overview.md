...
|---|---|
| S11, Windows — `git_common_dir` normalises git's forward-slash answer and every printed path is built with `os.path.join`; nothing here ran there. Round 1 found the other-drive shape (a linked worktree on another drive makes `os.path.relpath` raise) in the gate and the migration hook; both are fixed and pinned by an induced raise on every leg, and the real two-drive run is still nobody's. The windows leg of PR #95 (run 33643327406) then ran and failed once, on the test suite rather than the hooks: `tests/test_routing_is_recorded.py::test_the_second_prompt_names_it_too` still compared the gate's printed path against the `/` literal `seal/specs/<work-item-id>/routing.md`, where `declaration_hint` joins it with `os.path.join` and Windows spells it `seal\specs\<work-item-id>\routing.md`; phase 1 had moved only the first prompt's assertion to `os.path.join(...)`. That assertion now uses the same spelling; every other case passed on that leg (1347 passed, 22 skipped by their own platform guards) | CI's windows leg at the pull request |
