...
| Whether a re-stamped row's `Checked` date should move. `evidence-check --reverify` rewrites the hash and leaves the date, so nine rows in `seal/ledger.md` now carry 2026-09-03 over content from 2026-09-07, and round 1's fix pass added six more in the same state. Every one of those claims was re-read and still holds; moving the date would be changing a convention mid-work-item, which `CONTRIBUTING.md` asks a separate argument for, and doing it for some rows and not others would leave the file reading two ways | the repository owner |
| Whether E3's residual window is worth closing — the UTC midnight instant between a case computing the export's zip name and the export computing its own. It is microseconds a day where the defect was nine hours; closing it means planting a link at both candidate names | the repository owner |
