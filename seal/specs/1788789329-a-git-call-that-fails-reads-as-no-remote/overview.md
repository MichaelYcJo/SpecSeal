...
| Whether a re-stamped row's `Checked` date should move. `evidence-check --reverify` rewrites the hash and leaves the date, so nine rows in `seal/ledger.md` now carry 2026-09-03 over content from 2026-09-07. Their claims were re-read and still hold; moving the date would be changing a convention mid-work-item, which `CONTRIBUTING.md` asks a separate argument for | the repository owner |
