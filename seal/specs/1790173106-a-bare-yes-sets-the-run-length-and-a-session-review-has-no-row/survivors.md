...
<!-- What `survivor-check --range 659b4229..HEAD` reported at 4cd5a561 and why
each report is not a defect. Five of the six share words with `seal/ledger.md`'s
F8, which this branch REMOVED (its claim went with the code) and re-founded as
the fragment's B2; the sixth shares the shape of `run_reopened`'s old body.
The quote is the anchor: an exemption stops holding when the standing text
changes. -->
