...
|---|---|---|
| The class is enumerated from the source, not from a list | Given the module, when the walk runs, then every call and operator site in its syntax tree is classified | `test_the_walk_reaches_every_call_and_operator_site_in_the_module` |
| Membership is decided by what a site does, not by what it is called | Given a converter with a name nobody could have listed — a local `def`, an aliased import, a `from`-import — when the walk runs, then it is a member | `test_membership_follows_the_conversion_protocol_and_not_a_name` |
...
| The guard shapes the module uses are the shapes the checker accepts | Given the six constructed shapes, when each is classified, then the guarded ones pass and the ungarded ones fail | `test_the_guard_shapes_are_decided_by_what_they_catch` |
| The residual has one shape a next editor can act on | Given the sites the walk cannot resolve, when they are listed, then every one is a call on a receiver the module does not bind at module level | `test_the_unresolved_sites_all_have_one_shape` |
