...
|---|---|---|---|
| 1 | The checker: the walk, the two probes, the discharge analysis, and the cases | `./bin/test tests/test_a_derived_number_reaching_an_int_carries_a_guard.py -q`, plus the unguarded-site mutation seen red | |
| 2 | The rule reaches a next editor: `token_thirds`' docstring names the class, the checker and the two shapes the property does not reach, pinned by a case | `./bin/test tests/test_a_derived_number_reaching_an_int_carries_a_guard.py tests/test_session_cost.py -q` | |
| 3 | The records: ledger fragment, changelog fragment, questions, phase records, overview, and the `#192` box in `docs/flow.md` | `evidence-check`, and the suite module unchanged | |
