...
· evidence: `seal/ledger/1789687448-a-tracked-file-the-tree-deleted-stops-the-sweep.md` — 5 rows added; 8 rows of `seal/ledger.md` re-read, re-stamped and re-dated (9 anchors); 3 stamps in this work item's own `spec.md` re-stamped
...
stopped six of this suite's sweeps at the first missing path and every file
...
| How many shared ledger rows drift | `spec.md` §*Data & interfaces*: three anchors | Nine anchors in eight rows, plus three stamps inside `spec.md` itself | `evidence_check --strict .` at exit 2 named nine. Phase 4's sixth helper and phases 2–3's restructuring moved units the frame did not list; every claim was re-read and holds, and `phases/phase-5.md` lists them one by one. The three inside `spec.md` are what the records arm asks of a stamp in a live work item |
