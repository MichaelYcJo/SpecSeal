...
| PR | 380 |
| Broad gate | be86f74 against origin/release/v0.11.3 |
...
| 8 | ⬜ round 1's finding 8 — the proof block recorded a stale module count | `seal/specs/1789296300-a-segments-own-wall-clock-is-in-no-column/overview.md` | answered | The count was true of the run that wrote it: `f63db1c` recorded 104 and `4af3441` added a case two commits later. Correcting it now would make round 2 close on a fix, which spends the one reopening that remains and calls a third round for a test count — the arithmetic this release already paid for once on the sibling branch for #354. Nothing reads the proof block's number, and round 3, the sealer, or the next session that opens `overview.md` can move it for free |
...
| 10 | ⬜ the follow-up row enumerating where #145's median reached omits `tests/test_session_cost.py:2066` | `seal/follow-up.md` | answered | Same grounds. The `seal/follow-up.md` row is the owner's list and it is short one reach point, `tests/test_session_cost.py:2066`; the row is theirs to extend when they take it, and the item is deferred to them already |
| 11 | ⬜ `survivor-check` over the fix range reports clean with or without `--exempt`, so the exemption record cannot be re-derived | `seal/specs/1789296300-a-segments-own-wall-clock-is-in-no-column/survivors.md`, `skills/code-review/scripts/survivor_check.py#corrected` | answered | Not this branch's defect and not fixable here: `survivor_check` builds its corpus from the range's own diff, so a quotation the range itself wrote drops out of what it searches, and the exemption record therefore cannot be re-verified by the command. That is the same corpus-versus-range shape as #308 and #371, which are already the range half and the corpus half of one fix in `backlog: gates & hooks`. Recorded here so the next reader of `survivors.md` knows the command will not confirm it |
