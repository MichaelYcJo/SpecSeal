...
*reachable at all three places it is invoked* where the corrected text said
*the three places that invoke it … carry its full path*, and the shared
phrasing fell under the threshold. The check found three copies and the class
was four, which is `skills/agent-contract/SKILL.md` §12 applying to the tool
...
| 4 | `changelog.md` and `seal/ledger/1789338080-….md` fragments, and `overview.md` | the two hygiene-adjacent modules green, `evidence-check --strict .` clean on the fragment, `uvx ruff check` and `ruff format --check` on the one new test file | 9693c3a |
