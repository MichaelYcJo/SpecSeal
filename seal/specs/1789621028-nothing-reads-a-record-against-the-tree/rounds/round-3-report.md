...
<!-- Orchestrator, 2026-09-17: the eight rows carrying round 2's closures forward were written with a number in the `#` cell and round 2's own verdict word. Both columns are put into the shape this tree uses for a carried closure — a bare severity marker and `confirmed` — because `chain_check.closed_with_a_fix` reads every verdict cell, so inheriting `fixed` made round 3 read as a record that closed on a fix and the cap refused it. Measured: 3 of the 71 work items with two or more records have a last record of that shape. No finding, coordinate, grounds or terminal line was touched, and the reviewer's judgments are as it wrote them. -->
