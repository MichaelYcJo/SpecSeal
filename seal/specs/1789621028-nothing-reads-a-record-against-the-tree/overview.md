...
| The `isdigit()` half of `close`'s count guard is pinned by no case, and this pass may not add one. Reverted alone it is 102 passed, exit 0 | the repository owner, as its own work item. **A case cannot be added here and the refusal is the generator's, not a judgment**: it would be a unit in `tests/test_the_fixes_close_the_record.py`, which is the file whose one unit `rounds/round-1.md`'s `New units` names, and round 2's 🟡 2 — the finding this pass closed — sits inside that very unit, so `close` refuses the pair at depth 2 before writing any cell. The rule is `skills/code-review/orchestration.md` §*A fix pass adds the unit that pins it*, and `tests/test_the_fixes_close_the_record.py#test_a_depth_two_refusal_names_the_finding_whose_fix_added_the_unit` is the case that drives it — so this is the generator's refusal read off a shipped case, not a judgment made here. It is a new refusal path rather than a cosmetic change, which is why it is written down here rather than left |
| The `\| Fix range \|` row's closing pipe in `templates/sdd-round.md` is pinned by no case. Reverted alone it is 181 passed, exit 0 over the three modules that read the template | the repository owner. What would pin it is a sweep asserting every field row closes — a **walk**, which is named in the list of what a fix pass may not add. The cost today is zero: `split_row` tolerates the open cell, so the template is a form people copy rather than a thing that breaks |
...
grounds are the 12 sentence forms measured across 15 files, by the command
`docs/review-chain-spec.md` §*The fix range* names. A parser over that is a
