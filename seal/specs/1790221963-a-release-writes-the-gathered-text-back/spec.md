...
| The records | Ledger rows F1 and C2 corrected in place with a dated note. They say the `lost` guard is what keeps a gathered release's text out of `written`, which H1 and H2 show false. Step A's `plan.md` gate row gains the clause #557 supplies. Every row the design drifts is re-read and re-stamped, new rows go in this item's own fragment, and the changelog fragment, `overview.md` and the branch's own sweep are done | Phase 4 |
...
| Restricting the held set to fragments newly gathered in this range | Every marker at `b` is read, including earlier releases'. That only drops from `written` sentences nobody in this range wrote, which is the noise direction. It costs nothing measurable and saves a second `CHANGELOG.md` read (`plan.md` §*Alternatives*) |
...
7. **Step A's records are corrected in place, not left.** F1 and C2 are live
   ...
   which `correction-check` reads. Step A's `plan.md` gate row is corrected
   the same way, with the clause #557 supplies. Round records are not edited:
   they are records of a past state.
