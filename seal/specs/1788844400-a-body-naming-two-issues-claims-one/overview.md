...
| Whether the step behaves on a real GitHub runner — that `github.event.pull_request.body` arrives as an empty string rather than absent for a pull request with no description, and that `::warning::` renders as an annotation on the job. Both are read from GitHub's documented behaviour and from the workflow file, not executed | CI, on this pull request |
