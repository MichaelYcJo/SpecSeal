...
| 5 | The masking gives up five well-formed shapes the plan does not enumerate, beyond the unclosed fence it does | `.github/scripts/issue_claims_check.py:123` | deferred #266 | #266, opened for it after round 2's ⬜ 8 found this cell and the §Deferred row pointing at each other. The five shapes are enumerated there |
...
|---|---|---|
| Whether `FENCE` and `SPAN` in `close_issues_on_release.py` should cover tilde fences, four-space indented blocks, fences indented inside a list item, HTML comments and double-backtick spans — which changes what a release closes, not only what this check reports | **#266**, and a `# RIDER:` at both patterns in `.github/scripts/close_issues_on_release.py` so the file itself names it — round 2's ⬜ 8 found this cell and finding 5's verdict pointing at each other, and round 3's ⬜ 12 found that an issue alone leaves whoever opens that file meeting neither | the repository owner |
