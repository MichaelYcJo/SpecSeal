...
| Q5 | Nothing counts how often `--written-late` is reached for. The guard against reaching for it every round is a sentence in `skills/code-review/orchestration.md` §*It is an answer, not a way around the sequence* — should it become something a check can see? | **a person** — the repository owner | **Leave it a sentence**: the relaxation costs nothing and a run that leans on it every round is visible only to whoever reads the records, which is the same party the sentence addresses. **Count it**: a check reading how many of a work item's records carry `Written late` and printing or refusing past some number. That is new mechanism, so it is an issue and a work item of its own — round 1's fix pass may not build it (`skills/code-review/orchestration.md` §*A fix pass adds the unit that pins it*) | **leave it a sentence** — round 1 raised it as a question rather than a defect, `plan.md` already names it as this design's own six-month failure scenario, and the gate-change disclosure in `overview.md` states the direction honestly | ⬜ |
| Q6 | Does a repair path a message ADVERTISES owe a case of its own? The refusal `written_late` now prints tells a reader to add a `\| Written late \| yes — <why> \|` row by hand and commit it. Round 2 of #345 drove that path end to end — exit 1, row added by hand and committed, exit 0 — so nothing ships unproven; what has no case is the promise itself | **a person** | (a) **a case pins the advertised path**, so a later edit to the message or to the reader cannot silently make the printed instruction false · (b) **no case**: the path is exercised by the cases that already cover the row, and pinning prose a message contains is how a test becomes a transcript of a paragraph | **(b)** — nothing is unproven today, and the question is whether a printed promise is a contract rather than whether it currently holds | ⬜ |
