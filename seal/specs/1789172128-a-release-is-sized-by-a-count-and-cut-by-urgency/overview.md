...
| whether `hits()`'s joined-line branch survives a later edit. Measured: reverting it leaves the module green at 7 passed, and a case pinning it is depth 2 — round 1's own fix pass created that unit | the repository owner, via the row in `seal/follow-up.md`. It is the one half of round 2's finding 2 that this pass may not close |
