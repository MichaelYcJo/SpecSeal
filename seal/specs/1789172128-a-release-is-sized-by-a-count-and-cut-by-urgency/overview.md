...
| whether this range leaves any removed wording standing. `bin/survivor-check` answers *no removed wording is still standing* over this range with `--exempt` and without it alike, and round 2 measured why: `corrected` counts a review record's verbatim quotation as wording the range wrote, so the check is disarmed from the commit that posts round 1. **No reading this work item has of that check is evidence** | the repository owner, through **#365**. The four `survivors.md` rows are kept and are armed again when it lands |
| whether `hits()`'s joined-line branch survives a later edit. Measured: reverting it leaves the module green at 9 passed, and a case pinning it is depth 2 — round 1's own fix pass created that unit | the repository owner, via the row in `seal/follow-up.md`. It is the one half of round 2's finding 2 that this pass may not close |
