...
**Every row below excuses nothing today, and none of them was ever printed
under `exempt` after round 1 posted.** Round 2's finding 3 is why: `corrected`
does not apply to the range the `rounds/` exclusion the pool applies, and a
review report quotes the defective wording verbatim, so committing a round's
own record subtracts the phrases that round reported. Measured at `95b3d83`
with `--exempt` and without it — exit 0 and the identical line *no removed
wording is still standing* both times, which is the only honest thing the
command can be said to have told this work item. The checker is **#365**.

The rows are kept rather than deleted. They are the judgments a person made
about four real survivors, reported at `b46ff77`, and repairing the checker arms
them again. All four came from removing one ledger row — S3 of
`seal/ledger/1789100139-…md`, whose claim named the sizing rule's own wording.
Three are another work item's records and the fourth is a phrase collision.
