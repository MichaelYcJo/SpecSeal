...
**No unit added, and that is the gate's doing rather than the pass's.** The pass added two cases at what it read as depth 1 and refused a third at depth 2. `round_record.py close` refused all three at exit 2, keying on finding 2's `Location` rather than on what each case pins, and wrote no cell. The owner's answer was to take the gate's exit, so the two cases and their `NOUN_FORMS` constant (NAME NOT IN TREE) were reverted, two now-dead coordinates came out of ledger row R1, and the work went to **#366**. The three widenings those cases would have pinned are still in the tree and ship unpinned — which is the cost of the exit, stated rather than absorbed.
