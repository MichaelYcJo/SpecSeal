# 1790297083-the-release-job-goes-red-on-a-record-it-need-not-judge — routing
...
| Branch | fix/598-the-release-job-goes-red-on-a-record-it-need-not-judge |
...
Answered 2026-09-25 by the repository owner, before the first edit.

## Why this way

#598 (instances 1 and 4) and #529, how `chain_check` reads a record: a record byte-identical to the one on `main`, `Pass` beside `nobody` in the documented window, and a record deleted and re-added on a side branch. The owner pressed `automation` once for every work item of `release: 0.15.4` in one question.
