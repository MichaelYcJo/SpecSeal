...
| Pins beyond item 9 | `spec.md` item 9 names two new pins | a third: `test_letting_drift_warn_takes_both_halves` runs `evidence-ci`'s recipe over a malformed row, and round 1's fix pass added a fourth, `test_every_other_page_that_grades_the_flag_names_malformed` | the recipe's behaviour was red on the phase-1 checker. **Corrected 2026-09-26 by round 1's fix pass:** this row said the third case pinned step 4's new sentence; it pinned the recipe's behaviour and no sentence, so step 4, `SKILL.md`'s `--strict` row, the template's comment and the module docstring stood unpinned (round 1's 🟡 2). The fourth case holds all four against `exit_code`, red with each document restored alone |
