...
| PR | not yet opened |
| Broad gate | passed after `release/v0.9.1` was merged in — 2714 passed, 2 skipped; `ruff check .` and `ruff format --check .` both exit 0. CI's Windows leg had failed one case before that merge and it was a real defect: the local root printed with two separators in one path, normalised at the source in `optin.home_paths` |
