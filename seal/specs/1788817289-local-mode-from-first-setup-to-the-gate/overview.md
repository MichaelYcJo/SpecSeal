...
| ✅ `tests/test_the_pull_request_language_is_the_repositorys.py#items` reimplements `hooks/config.py#config_rows` rather than calling it — a third copy of one parser | closed 2026-09-17 by work item 1789598366 (#415) phase 3: the copy is removed and that name is now an alias for the one reader, asserted by `__code__.co_filename`. #415 is what forced it — the production reader learned markdown's `\|` escape and a copy that had not would have answered a different question about the same file |
