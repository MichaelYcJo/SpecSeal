...
| `seal/specs/1789598366-a-piped-broad-gate-row-takes-every-config-row-below-it/survivors.md` | the sentence holds only where a row above already parsed | **This file's own grounds cell, and still true.** What round 2's fix changed is which line the gate's message is built from; the stop rule is untouched — `config_rows` still breaks on a line it cannot parse only once it has found a row, which is exactly what that grounds sentence says #415 answered |
