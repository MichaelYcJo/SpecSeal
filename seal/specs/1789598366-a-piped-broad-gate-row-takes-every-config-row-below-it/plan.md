...
| **No compatibility break for a repository whose config parses today** | ~~A widened cell pattern can only make more lines into rows, never fewer.~~ **That generalisation is false and round 1 🟡 2 measured it.** The pattern narrows in exactly one shape — a backslash immediately before a cell-ending pipe, which used to be a plain character followed by a delimiter and is now one escaped pipe. A6 is the acceptance scenario that asks for the measurement rather than the argument, and the measurement holds: no file in this tree reads differently through `config_rows`. What the argument could not deliver is what M3 was for, and the corrected shape is named in `spec.md` §*What this repair cannot see* |
