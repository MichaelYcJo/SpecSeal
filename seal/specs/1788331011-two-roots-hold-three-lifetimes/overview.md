...
| ✅ `hooks/root-migrate.py` on Windows — porcelain's two columns and the `/`-joined pathspecs are written for it, nothing here ran there | CI's windows leg on PR #90, executed: every test of the hook green (`https://github.com/MichaelYcJo/SpecSeal/actions/runs/33628905815`). The two failures on that leg were the by-hand README test, which ran a `bash` block through `cmd.exe`; it runs under `bash -c` now and skips where no usable bash exists |
...
| ✅ case-insensitive filesystems: `tracked_names` compares git's exact names, and nothing here ran on one | the macOS leg of the same run, `pytest (macos-latest, 3.12)` pass, and the hook's tests on its windows leg (`https://github.com/MichaelYcJo/SpecSeal/actions/runs/33628905815`) |
