...
| 🟡 H | `specs/` ITSELF as a symbolic link is 🟡 A on the other root: the home moves, the work item is called "not tracked as a SpecSeal work item" and left, its rows are re-pointed to `seal/specs/<id>/…` which does not exist, the marker is stamped, the next start is silent | `hooks/root-migrate.py#main` (the `.specseal` link check names `OLD_HOME` only), `#repoint_path` (re-points without asking whether the unit moved), `spec.md` refusal table (the link row names only `.specseal/`) | fixed — bb6c5e3 (code, test) and 9717028 (spec, the records); the refusal sits after the scratch check so a throwaway repository keeps its own line | executed at HEAD: `moved .specseal/ into seal/ (3 ledger rows re-pointed; left specs/1788000000-a-work-item … (not tracked as a SpecSeal work item))`, stamped, checker `2 ok · 2 broken`, second start silent |
