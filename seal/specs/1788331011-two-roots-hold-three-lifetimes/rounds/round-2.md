...
| PR | not yet opened |
| Broad gate | not yet |
| Fixes checked by | nobody — the fix pass for 🟡 A and 🟢 B–E had not run when this record was written; round-3 is the verifying round and sets this cell |
...
| 🟡 A | a symlinked `.specseal/` is skipped without a word: git tracks the link as one blob, so `tracked_names` returns nothing for the home, the work items move, the marker is stamped, and `seal/ledger.md` never appears — the checker then finds no ledger. round-1's reading ("fails like 🔴 1") was wrong: it does not fail, it half-moves and reports success | `hooks/root-migrate.py#moves` (the home units), `#main` (the stamp) | open — fix pass follows: refuse before `dirty()` when `.specseal` is a link, naming the by-hand section, no stamp; test `test_a_symlinked_specseal_is_refused_not_half_moved` | executed on a fixture (round-1 ❓ b) |
| 🟢 B | "when git cannot answer, the dirty test refuses" is not enforced by code: all-git-down is silence (`optin.repo_root` returns None first); `ls-files`-only-down moves from the directory listing and 🔴 1's shape returns | `#entries`, `#dirty`, `spec.md` step 4, the fragment's 🔴 1 row | open — fix pass follows: `dirty()` returns True when either `tracked_names` is None; spec step 4's last sentence says so | executed with a patched `git()` (a contrived state) |
| 🟢 C | `old_items` lost the `isdir` filter; an item-shaped tracked FILE under `specs/` moves as a work item, against §"Which entries of `specs/` are SpecSeal's" | `#old_items` | open — fix pass follows: restore the filter | executed |
| 🟢 D | a file at a directory's destination reports `cannot create seal/` naming the wrong parent | `#git_mv` | open — fix pass follows: name `os.path.dirname(dst)` | executed |
| 🟢 E | `by_hand_block` takes the section to end of file; a lost block would run the next bash block in the README under `shell=True` | `tests/test_the_root_migrates_itself.py#by_hand_block`, `#HEADINGS` | open — fix pass follows: bound the section at the next `## `, assert the block, full heading for the Korean README | executed on an edited copy |
