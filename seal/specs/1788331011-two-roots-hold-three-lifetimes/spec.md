...
| `.specseal/` is a symbolic link | `specseal: .specseal/ is a symbolic link, which git tracks as the link and not as its files — not moving it. Move by hand (README, "Coming up from 0.3.x") and remove the link.` Tested first, before the marker and the dirty test, so it prints at every session start until the link is gone; a link's units are none, so the alternative was the work items moved and the ledger left behind under a stamp. The same line prints on a migrated repository that kept `.specseal → seal` as a link for old tooling: that is not a half-move, nothing old is behind the link, and the line is the cost of testing the link before the marker (round 3, 🟢 I) | not stamped |
| `specs/` is a symbolic link holding work items | `specseal: specs/ is a symbolic link holding work items, which git tracks as the link and not as its files — not moving it. Move by hand (README, "Coming up from 0.3.x") and remove the link.` Tested after the unit and marker checks, so a migrated repository stays silent, and before the dirty test; only a link holding at least one item-shaped directory is refused, because a linked `specs/` holding none is not SpecSeal's name. A link's units are none, so the alternative was `.specseal/` moved, the work items named as left where they are, their rows re-pointed to a `seal/specs/<id>/` that never appears, and all of it under a stamp | not stamped |
...
The rows are tested in the order: the `.specseal/` link, nothing old left,
marker, scratch, the `specs/` link, dirty, then the move. The last row is the ledger hook's rule applied here,
