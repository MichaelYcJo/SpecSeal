...
| 7 | Round 3's eight 🟡 as four classes and two singletons — the declared limit widened to what actually escapes, the arm's key stated where a reader meets the refusal, the terminal `Fixes checked by` value put in the spec's table, and every re-stamped ledger row carrying what its re-read found | eighteen mutations across every decision this branch argues in prose, nine surviving and five of them closed here; nine cases written or widened and every one seen red; 559 cases green across the sixteen suites that read the two checkers or the documents this phase edited | `e94c3de` |
