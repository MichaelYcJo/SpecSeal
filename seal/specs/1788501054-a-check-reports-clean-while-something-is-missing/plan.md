...
|---|---|---|---|
| 1 | `docs/review-handoff-protocol.md` and `skills/code-review/SKILL.md` name the two forms and what each is for — the unscoped read that says what a branch broke, the scoped write that keeps `--reverify` off somebody else's row | cases pinning both sentences, seen red with the prose stashed | |
| 2 | `evidence_check.py` says which ledgers it did not read when `--ledger` narrowed it | a case on the output, seen red before the line exists | |
