...
| Contract changes | `fix_surface` gained an arm and a second cutoff, its return shape unchanged → `main` and `tests/test_chain_check_at_the_pull_request.py` line 1301, which calls it directly over every real round record in this repository; `skipped_by_narrowing` now falls back on a zero `st_ino` as well as on `OSError` → `main` and `tests/test_a_narrowed_ledger_read_says_what_it_skipped.py` line 260, which calls it in-process because the platform state cannot be produced from outside. **Corrected in round 3's fix pass** (🟡 8): this cell named `main` as the only call site of each, and a reach not revisited is #57's largest regression class exactly — the class this row exists to close |
