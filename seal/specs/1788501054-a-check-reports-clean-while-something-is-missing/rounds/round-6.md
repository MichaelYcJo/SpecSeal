...
| 🟡 4 | The four calls about what to leave standing — three hold | four records | answered | The fourth is 🟡 1, fixed at `0d4a31f`. `phase-8.md` left whole is right: a phase record is the account of one commit, and `phase-9.md` is where a reader learns it was reverted. `round-1.md` and `round-2.md` are correctly repaired, and `round-1.md`'s re-attribution of the reason to `chain_check.py` is true |
