...
| PR | #162, opened ready at `3bf0fd6` and returned to draft when 🔴 8 arrived — this cell said *not yet opened* until round 13's 🟡 6 |
...
| Whether `st_ino == 0` **arrives** on `windows-latest` — still open. What the leg's run at `3bf0fd6` (33939786295) showed is what the checker does with a zero: the cases that **build** one ran there and passed, and a built zero cannot show that one arrives. This row called that *answered* for one round — round 13's 🟡 5 asked the closure to land and round 14's 🟡 3 found the closure did not reach the question — and `overview.md:51` had kept it open the whole time. A green leg can never close it: a platform that never produces a zero never reaches the branch, and the fix is correct either way | `overview.md:51`; this row | the windows CI leg, as before |
| `questions.md` Q2, Q3 · issues #158–#161 · issue #160's intermittent four | as before | the repository owner |
