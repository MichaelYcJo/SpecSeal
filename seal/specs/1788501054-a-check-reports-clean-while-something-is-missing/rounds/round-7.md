...
THE FIX SURFACE ABOVE is the reach-back, filled at `e52d641..50e9154`, and
the depth reason sits here rather than in the cell, which is round 4's whole
lesson. `wrote_fixes` answers a finding inside `stopping_floor`, which work
item `1788472135` created before this run began, so it is depth 1; the three
units in the floor suite answer the same finding, in a file whose `record()`
