...
| How many sites there are | `plan.md` §Technical context: *"four subtractions, one class"* and *"`data['command_s'] / data['span_s']`, and **three more divisions** by the same denominator, including the `idle > data['span_s'] * 0.1` guard"* — four of each. The code: **six** subtractions plus **two orderings**, and **three** divisions | The implementation, and the record corrected | An AST walk over the module returned 56 arithmetic and ordering sites and classified each; the two the subtraction list missed are the `exact`/`stripped` duration and `slowest`'s, and the orderings are `load`'s `calls.sort` and `analyse`'s `max(turn_end, …)`. The fourth division the plan counted is `idle > span_s * 0.1`, a multiplication, safe at zero — which is why the early draft's conjunct on the idle block was removed before phase 1's commit. **Round 2 measured that removal wrong**: safe at zero is not safe at a negative span, where the threshold is negative and the comparison holds, so the report printed an idle figure beside a dash. The conjunct is restored, and the site is a third independent test of the span rather than one `share` closes. `phases/phase-1.md` §*The enumeration* carries the walk; `plan.md` is left as approved, since it is the contract this work executed against rather than a description of the result |
...
| Whether any other operation reconstructs a value outside the range from operands that each passed a funnel. `count` answers for what enters and `token_thirds` is now guarded for what it computes, but enumerating call sites says which operations exist and never which values reach them — round 3's 🔴 arrived through a call the walk had listed since its first widening. Recorded as a limit rather than claimed closed, and handed over as an issue | issue #192, which this run filed; the fragment's R3 carries the method |
