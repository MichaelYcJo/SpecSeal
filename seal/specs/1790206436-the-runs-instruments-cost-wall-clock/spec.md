...
lists. A new `has_xdist(venv)` reads the filesystem only. A new NAME NOT IN TREE
...
**`skills/verify/scripts/broad_gate.py`.** A new `shipped_gate(root)` returns NAME NOT IN TREE
...
`tests/test_a_parallel_agent_names_its_scratch_after_the_work_item.py` (A12–A14, NAME NOT IN TREE
