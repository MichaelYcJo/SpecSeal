...
· verified: executed — the module's 69 cases at each phase boundary, 13 cases seen red first, 8 mutations red, the three pull-request ranges measured three times with and without `--exempt` and with the file deleted in a scratch clone, the branch's own sweep, `evidence-check --strict`, `unverified-check --baseline`, ruff on the touched files; read — the four tickets, the review-chain spec's folded statements, the module's other readers of a path (S8); unverified — the table below
...
| S6's pool half, red direction | `spec.md` S6: *for the pool half, a phase record reported as a survivor* / seen red at exit 0 with nothing reported | the case pins both | A fourth carrier halves the weight of every phrase it quotes, so on the frame's own four-file fixture the record dilutes rather than reports. The case asserts exit 1 and that `guide.md` alone is named, which refuses both directions. `phases/phase-2.md` |
...
| The `Ran by` cell of `phases/phase-1.md` through `phase-4.md`, written `unknown` because the spawn prompt handed over no agent or model name | the orchestrating session, which knows what it spawned |
