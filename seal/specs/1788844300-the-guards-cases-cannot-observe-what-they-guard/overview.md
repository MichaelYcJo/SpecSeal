...
| Round 3's own mutation count | Its `## Executed probes` row reads *six mutations, one per arm — 4 red, 2 green, `spec.loader is None` and `SyntaxError`*, which counts `ImportError` as covered | Corrected the record in place to 3 red, 3 green | Executed against `tests/test_chain_hooks.py` as it stood at `c3c22c2`: deleting `ImportError` from the except tuple leaves 27 passed, exit 0 — the same reading `SyntaxError` gives. Corrected in place because that is what this repository did to `rounds/round-2.md`'s grounds cell at `b73bdd3`, and a false measurement left standing in a record is the class this branch is about |
