...
|---|---|---|
| `zipfile.ZipFile.extractall` for the import | **It overwrites**, which is the one rule this command exists to keep. It also writes members this format has no place for, and it follows a symbolic link that is already a directory in the destination — the only escape that measured (`spec.md`, *What `extractall` does and does not do*) | **Rejected**, but not for the reason this row first gave. It claimed traversal, and the measurement on 2026-09-03 said today's CPython strips `..` and a leading `/`. The name validation stays anyway, as defence that does not depend on that sanitiser |
