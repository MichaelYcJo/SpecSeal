...
| A zip larger than memory: `write_members` reads each member whole with `archive.read(name)`, which is fine for markdown records and is not bounded | the repository owner, if a root ever holds something that is not a record |
