...
| The manifest declares a `format` this build does not read | the whole zip is refused, naming both numbers, before any name check | A later format is exactly what moves the names, so checking them first answers a build-too-old zip as a malformed one. The field exists for no other day |
| The manifest gives `head`, `exported_at` or `remote` as something other than text | the field is read as absent rather than raising | Those come from a manifest another machine wrote, and `read_manifest` checks the object and its `format` and nothing else. `head` without `exported_at` used to raise at the closing line, after every record was written |
