...
| S15 · #222's paragraph is in the docstring and the rows are re-stamped | `depth_two`'s docstring names the by-file scoping, says a case in another file is never a candidate, and names contract §15; the three rows on `depth_two@8c7bfa47` are re-read, noted and re-stamped | read; `evidence-check --strict` exit 0 |
...
heading), the #222 paragraph (on `depth_two`), the #55 sentence (on
