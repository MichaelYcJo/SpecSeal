...
| 6 | **The records.** G6 and G7's `Code grounds` widened with a `Corrected <date>` note and `--reverify`; the fragment written in one pass; `changelog.md` (one entry per closed ticket, #331 named as deferred); `overview.md` with `## Not verified` naming the sealer for the broad gate; `survivors.md` for the rewordings the sweep still reports; `questions.md`'s W and M rows filled | `evidence-check --strict .` exit 0; `bin/survivor-check --range origin/release/v0.15.1...HEAD --exempt <this item>/survivors.md` exit 0 with every `exempt` line naming a row; `bin/unverified-check --baseline origin/release/v0.15.1 seal/specs/` exit 0; `python3 skills/evidence-check/scripts/correction_check.py --range origin/release/v0.15.1...HEAD` exit 0; every exit read directly || `03896403` |
| 7 | **#561, the checklist's before-and-after ledger count** (added at the resumption). Step 2's second block takes the table-line count and the strict exit before `--split`; §3 compares against them with one glob-free `find … -exec cat {} + 2>/dev/null \| grep -c '^\|'` in a fence, and the sentence after the fence has a subject. Lands before the 0.15.1 release runs step 2 | the count and the strict exit in zsh on a scratch clone, before `--split`, after it and after `--version`, equal; `bin/test tests/test_docs_line_wrap.py -q`; `evidence-check --strict .` | `eaba2dd1`, with its drifted rows re-read at `31937b9f` |
| 8 | **#562, the unescaped pipes** (added at the resumption). Every `\|` inside a ledger cell of the rows whose cell count exceeds their header's is escaped, text otherwise byte-identical; a case in `tests/test_release_hygiene.py` refuses such a row over the shared file, the release files and the fragments, seen red first | the case red against the unescaped ledger and green after; the files byte-identical with the escapes removed; `evidence-check --strict .` | `28439296` |
