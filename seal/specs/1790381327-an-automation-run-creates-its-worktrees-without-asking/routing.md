# 1790381327-an-automation-run-creates-its-worktrees-without-asking — routing
...
| Branch | fix/604-an-automation-run-creates-its-worktrees-without-asking |
...
#604 and #8, the worktree guard in an automation run: its first `git worktree add` asks a person right after the owner pressed `automation`, and it counts sessions where a subagent is not one. The owner pressed `automation` once for every work item of `release: 0.15.5` in one question.
