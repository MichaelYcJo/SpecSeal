...
| Q3 | A switch written after a creation in the same command is never judged (`main`'s walk skips it once the creation has set the verdict), so with consent present `git worktree add … && git switch x` goes silent even over another session's ACTIVE tree. This has held for every session with a consent record since #257; this work extends it to the first creation of an automation session | the work | Not built here: its fix belongs to the walk, not to what counts as consent, and the milestone says this release adds no mechanism. The orchestrator files it as an issue before the pull request, and the PR body names it | Out of scope; filed by the orchestrator | ⬜ |
