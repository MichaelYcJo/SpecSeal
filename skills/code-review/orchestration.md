...
held, which stays behind it as `earlier run`, unless that run is the same
...
entry by either writer, and one at the same commit against the same base is
replaced by either — and it is for the one case `seal` refuses by
