...
`chain_check.py` reads that row on **every** record — §Data & interfaces says
why the scope differs from `Pass`'s — and refuses a claim git contradicts: a
missing row, a round that does not exist, one that is not later than this one,
one whose own `Target SHA` is not later either, and `no fixes to check`
written beside a verdict that closed with a fix. Round 1 of this work item's
review added one refusal that is not a claim git contradicts but a state: a
checked `Pass` beside `nobody — <why>` on the last record, for work items
begun after the rule landed. That is Q1, answered by the repository owner.
...
**In after all, and it arrived as an answer rather than as scope.** The first
version of this section had `chain_check.py` never failing a record whose
`Pass` is checked beside `nobody — <why>`, with the cost priced as Q1 and the
repository owner as its answerer. The owner answered during round 1 of this
work item's own review, and with a third option: refuse it, for work items
begun after the rule lands, and grandfather everything before. Both halves of
the trade the row priced are kept — the refusal, and no red on merged history
nobody can repair.
...
|---|---|---|
| A record that claims a checker it does not have is refused | Given a last round record whose `Fixes checked by` names `round-4` · When no `round-4.md` is committed · Then `chain_check.py` exits 1 naming the round it could not find | Executed — `tests/test_the_last_rounds_fixes_are_checked.py` |
...
| `round-N` whose own `Target SHA` is the same commit as this record's, or an ancestor of it | **fails** — the number is later and the review is not, so it read none of the fixes it claims to have opened |
| `no fixes to check`, and no verdict cell in this record closed with a fix | passes |
| `no fixes to check` beside a verdict cell reading a fix word | **fails** — a contradiction inside one file, the shape this check already refuses for `Pass` beside an open 🔴. The cell is read through markdown emphasis and the commit citation after the word, because `**fixed** \`sha\`` is how every record in this repository spells it |
| `nobody — <why>` | prints the state on every run. **Fails** beside a checked `Pass` on the last record, for a work item begun on or after `chain_check.py`'s `STRICT_FROM`; passes everywhere else |
...
request outright. **Answered** by the repository owner during round 1 of this
work item's own review, and with a third option the row did not carry: refuse
it for work items begun after the rule lands, grandfather the rest. Q2 —
whether the field should be read on every round record rather than the last
one. Q3 — whether there is a bound on how many verifying rounds a run may
have. Q4 — whether the round cap should be enforced in code at all.
