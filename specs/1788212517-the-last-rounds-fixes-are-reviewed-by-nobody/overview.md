...
| Whether the `Pass`-beside-`nobody` refusal fires on a real run rather than on a fixture. The three records in this repository are all grandfathered, so no work item here can reach it until the next one | the repository owner, at the first work item begun after this release |
