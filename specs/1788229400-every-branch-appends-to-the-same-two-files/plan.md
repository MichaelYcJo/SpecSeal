...
| 11 | A 0.1.0 ledger is loud, never invisible: `OLD-FORMAT` fails the run with or without `--strict`, and `--migrate` ships the same enclosing-unit migration this branch ran on its own 51 coordinates, leaving and naming what it cannot prove | today's silent `0 ok` pinned as the failing expectation first; idempotence and the all-or-nothing row pinned; 6 mutations, all killed | `ba147b6` |
| 12 | The migration runs itself at the first session start, once per repo, never over an uncommitted ledger file — `claude plugin update` becomes the whole of what a user does, with `--migrate` kept as the CI and by-hand fallback | each boundary seen red first; 9 mutations over every hook branch, all killed; the every-session scan measured at ~24 ms in-process, ~60 ms for the whole session-start group |`60dfb74` |
| 13 | Review round 4, the redesign span's first review: the generic rule stops reading `return f(x)` as a declaration; a parsing `.py` never falls to the text rule (ast carries constants instead); EXTERNAL demands declared cross-repo intent and the scan never searches a repo a row may not cite; `--reverify` reads `--default-repo`; the dedup key gains the hash; OLD-FORMAT reaches both totals lines and the commit advisory; heading paths heal; `--migrate` reads `--map`/`--default-repo` and checks cited ranges against the old stamp's commit via git, leaving what changed since; atomic ledger writes and None guards; six documents corrected | every fix seen red first — 20 new or changed cases across four files; 15 mutations, one per new branch, all killed after two were reworked to the true bug shapes; the real ledgers at `49 ok` | `38802bc` |
| 14 | Review round 5, the verifying round: the since-the-stamp proof reads the file under the root it was given; an unreadable ledger is reported rather than read as empty; the declaration keyword list may narrow candidates and never empty them, with the recorded hash breaking the tie; the ledger writer replaces the real file behind a symlink and keeps its mode; a coordinate is confined to the repository it is placed in; cross-repo intent is read per row; the session-start migration carries the unproven count; four documents and the ledger's own header stop claiming the checker never calls git | every fix seen red first — 15 new cases across three files; the fail-open arms and both containment arms mutation-checked, all killed; the real ledgers at `56 ok` | `43fab14` · `8f61cd6` · `4784d04` |
| 15 | Review round 6, verifying round 5: the declaration rule stops being asked to be right and reports how sure it is instead — a resurrected place that reconstructs nothing is BROKEN with the destination named, and `--reverify` refuses to write onto one; the `--default-repo` placement runs the containment test the other two branches run; `--reverify` answers every row it leaves alone; a term that could never fire is removed | 7 new cases seen red first; 6 mutations over every new arm, all killed; the real ledgers at `56 ok` | `6f564d3` |
| 16 | Review round 7, the last fix pass: a place the declaration rule is unsure of is named with the hash to record, so such a unit can be put in a ledger again; `--migrate` answers that place the way `--reverify` does unless the old stamp vouches for the cited lines; the call marking widens from the semicolon to the span, closing 🔴 J for Swift, Kotlin, Go, Ruby and Lua; `--reverify` stops contradicting the check and stops answering a flagged row with silence | 11 new cases, 8 of them seen red first and the rest mutation-discriminated; 12 mutations over every new arm, all killed after two pins were strengthened; the real ledgers at `58 ok` | `a43ef64` |
