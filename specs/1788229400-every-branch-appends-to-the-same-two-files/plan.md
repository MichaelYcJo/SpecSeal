...
| 12 | The migration runs itself at the first session start, once per repo, never over uncommitted `.specseal/` changes — `claude plugin update` becomes the whole of what a user does, with `--migrate` kept as the CI and by-hand fallback | each boundary seen red first; 9 mutations over every hook branch, all killed; the every-session scan measured at ~24 ms in-process, ~60 ms for the whole session-start group |`60dfb74` |
