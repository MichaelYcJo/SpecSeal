...
| Fixes checked by | nobody — the fix pass has not run yet; the verifying round sets this |
...
| 🟡 13 | The differential generator (26 wrappers × 70 statements against bash) lives in a session scratchpad, not the tree; `plan.md` says a next reader can re-run it. Its 26 wrappers are all one-line, which is why 🔴 2 and 🔴 3 escaped it. A guarded test (`usable_bash`, multi-line wrappers) is the regression net | `specs/…/plan.md:200-202`, `overview.md:162` | open | read; scratchpad file confirmed |
