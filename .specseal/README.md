...
away — a fixture made by hand to reproduce a gate decision, which used to cost
a waiver on every command — and in a repository anyone reviews it silently
...
There is no list of what is still to do here, and no directory for one.
`plan.md`'s Phases table is where the work records how far it got, in a Status
column whose closed value is the commit that closed the phase — a past state
someone can open, rather than a tick anyone can type.
