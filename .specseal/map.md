...
| Clause | Code grounds | Verified behavior | Checked | Notes |
|---|---|---|---|---|
| A turn is one assistant message carrying at least one tool_use, keyed by the message id with the row uuid and then the row itself as fallbacks — per-block counting pinned tools per turn at ~1.00 structurally | `skills/verify/scripts/session_cost.py:64-141` | **Executed**: six new cases in `tests/test_session_cost.py`, each shown red against the per-block code first — a two-call message read 1.0 where 2.0 is true, a split message was two turns, a keyless transcript is one turn per row (the old floor, never an inflated ratio), and a message's tokens counted once per call | 2026-09-01 `5d011f5` | Reads DRIFTED until re-stamped after the squash lands: the cited lines are this branch's own, and per `templates/sdd-plan.md` the Checked column never names a commit the branch itself made. The phase commit is in this work item's `plan.md` Status column, deliberately not here — a second hex word in this row would silently become its drift baseline. Review round 1 established three more parser facts (executed there, read here): a result row never creates a turn even when its uuid differs from its call's; an unresolved tool_use joins the turn count but not the call count and drags the ratio down — 0.5 on a synthetic, negligible at a 65-call scale; non-monotonic timestamps split one turn into two gap groups and overstate model time, which an append-only transcript does not produce |
| Model time runs from a turn's last result to the next turn's first call, so a batch's internal waits are the tools running, not the model thinking | `skills/verify/scripts/session_cost.py:148-171` | **Executed**: `test_the_gap_after_a_batch_measures_from_its_last_result` books 7s where the per-call reading booked 15s, and the split-message case holds the intra-turn wait at zero model seconds | 2026-09-01 `5d011f5` | Same content caveat as the row above, but this row reads OK today, not DRIFTED: drift is judged in baseline numbering (`skills/evidence-check/scripts/evidence_check.py:100-105`) and this coordinate is HEAD numbering, so the tripwire does not cover these lines until the row is re-stamped after the squash lands. The advisory wording forks at a ratio of exactly 1.0: above it, `most turns send a single call` replaces the one-at-a-time claim the number no longer supports |
| The contract-override refusal stands in both agent contracts with the 28-minute case named | `agents/smith.md:134-160`, `agents/warden.md:79-93` | **Executed**: `tests/test_broad_gate_rule.py` green on this tree — the refusal half, the disclosure half, the WIDENING line and the ambiguous-instruction default all pinned in both files | 2026-09-01 `5d011f5` | Issue #29 inherited a row from #27 asking for this to be added; it had already shipped with this history's initial commit, so this work item verifies rather than writes. Only the smith clause's line numbers moved, under the handoff pointer added above it |
| The batching expectation is stated where it pays and caveated where it cannot be met | `agents/warden.md:163-169`, `agents/smith.md:162-170` | **Executed**: the two contract cases in `tests/test_the_handoff_before_round_one.py`, shown red before the prose landed | 2026-09-01 `5d011f5` | Task shape dominates the corrected numbers — review rounds read 1.29-1.89 tools per turn and edit-test loops 1.08-1.17 — so the reviewer's contract carries the expectation and the smith's carries the serial-loop caveat |
| The handoff before round 1 and the progress channel are protocol rather than habit | `docs/review-handoff-protocol.md:267-318` | **Executed**: four cases in `tests/test_the_handoff_before_round_one.py`, each red before the section existed — coordinates rather than prose, the three labels, where to measure, the Status column and its stall signal, and the pointer at the meter | 2026-09-01 `5d011f5` | Draft 0.6. No conformance rule was added: a spawn prompt is not a file the protocol can check. The meter had sat unreferenced through a full day of measurements nobody took |
