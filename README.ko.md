...
```
smith 가 만든다 → verify → warden 이 시험한다 → 사용자에게 보고
      ↑                                      │
      └── 다시 만들기 (사용자 지시) ← 사용자 판단
...
warden 은 그 기록을 다시 도출하지 않고 감사합니다. 나온 보고로 무엇을 할지는
사용자가 정하며, 체인은 보고에서 멈춥니다.
...
게이트 다섯 중 둘은 조건이 맞아야 깨어납니다. 커밋 게이트와 리뷰 이력 알림은
레포 루트에 `_ai/` 디렉터리가 있을 때만 동작하고, 그 전까지는 침묵합니다. 이
디렉터리를 직접 만들 일은 없습니다. smith 가 그 레포에서 처음 일할 때 문서 구조를
`_ai/` 까지 함께 만들기 때문입니다.

나머지 셋은 조건 없이 동작합니다. worktree-guard 와 session-lease 는 모든 git
레포에서 돌고, lint-python 은 저장하는 모든 `.py` 파일을 고쳐 씁니다. 전역으로
...
만듭니다. 기준 커밋을 새긴 `docs/policies/<도메인>/_evidence.md`, 인계 규칙을
담은 `_ai/README.md` 가 그렇습니다.
