...
| 컴포넌트 | 역할 |
|---|---|
| **에이전트** — `smith` · `warden` · `scribe` | Who: 구현·리뷰·마이그레이션 사실 확인을 별도 컨텍스트로 분리, 각자 방법론 스킬을 사전 로드 |
...
```
smith 구현 → verify → warden 리뷰 → 리포트 보고
       ↑                                          │
       └── 수정 (사용자 지시) ← 사용자 판단 ←──────┘
...
원본 ↔ 신규, 기본값은 **원본 보존**)과 원본 사실 확인 전담 `scribe`
...
**알아서 로드되는 것:** 에이전트 `smith` · `warden` · `scribe`,
