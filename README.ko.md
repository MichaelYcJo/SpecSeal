...
**Specs, sealed** — 코드가 스펙에서 멀어지면 빌드에 빨간불이 켜집니다.

![SpecSeal 데모: evidence-check 가 스펙-코드 드리프트를 잡는 모습](./assets/demo.gif)

코딩 에이전트는 다 됐다고 말합니다. SpecSeal 은 그 말에 **도장**을 요구합니다.
도장은 셋입니다. 스펙을 실제로 읽어야만 찍을 수 있는 제작자 인장, 이것이 없으면
어떤 커밋도 게이트를 넘지 못하는 warden 의 검인, 그리고 스펙 조항마다 근거가
되는 코드 좌표를 적어 두는 원장입니다. 코드가 바뀌었는데 원장이 그대로면
빌드가 깨집니다.

도장 없는 주장은 없다. 시험 없는 도장은 없다. 검인 없는 머지는 없다.

Claude Code **플러그인**으로 배포됩니다. 원장과 드리프트 검사기, 인계 규약은
...
| **scribe** (에이전트) | 필경사 — 판단 없이 충실히 옮긴다. 원본 코드가 실제로 하는 일을 좌표로 가져와, 원장이 사실과 어긋나지 않게 한다 |
...
| CLAUDE.md 블록 | 상시 로드가 불가피한 ~15줄 (툴링·안전 규칙 두 가지·Git) |
...
세션을 넘어가는 기억은 세션이 아니라 저장소에 남습니다.
...
원장은 쌓아 두기만 하는 것이 아니라 **검사**받습니다. 스펙과 코드를 잇는
...
| 게이트 | 언제 | 무엇을 | 어디서 |
|---|---|---|---|
| commit-review-gate | `git commit` 직전 | 사이클에 검인이 없으면 확인창 (`[no-review]` 로 흔적을 남기며 생략 가능) | 루트에 `_ai/` 가 있는 레포에서만 동작하고, 그 밖에서는 아무것도 하지 않는다 |
...
| 명령 | 역할 |
|---|---|
| `python3 <플러그인>/skills/evidence-check/scripts/evidence_check.py . [--strict]` | 원장 드리프트 검사(데모 GIF 에 나오는 그 검사). 에이전트 없이 동작한다 |
...
원장 드리프트 검사(`evidence_check.py`)와, 위 표에 적은 게이트 전부입니다.
