...
| 게이트 | 언제 | 무엇을 | 어디서 |
|---|---|---|---|
| commit-review-gate | `git commit` 직전 | `.git/specseal-reviewed` 가 현재 HEAD 와 다르면 물어본다(`[no-review]` 로 건너뛰며 그 표시가 명령에 남는다). `docs/parity.md` 를 선언한 레포에서는 코드를 커밋할 때 이번 HEAD 의 `.git/specseal-parity` 가 없으면 함께 묻는다(`[no-parity]` 로 건너뜀). 둘 다 걸리면 한 번에 알려준다 | 루트에 `_ai/` 가 있거나 `docs/parity.md` 를 선언한 레포. 그 밖에서는 아무것도 하지 않는다 |
...
| 커밋 명령의 `[no-parity]` | 원본 대조 게이트를 한 번 건너뛴다. 마찬가지로 흔적이 남는다 |
...
- **게이트는 묻고, 막지 않습니다.** 커밋 게이트는 확인창을 띄울 뿐이고 승인하면 그대로
  통과합니다. 명령에 `[no-review]` 를 넣으면 아예 건너뜁니다. 리뷰 없이 커밋하려는
  에이전트에게 과속방지턱이지 보안 장치가 아닙니다. 확인창을 승인할 수 있는 사람이면
  누구나 커밋할 수 있습니다.
...
- **증명 블록은 강제가 아니라 공개입니다.** 어떤 훅도 그 내용을 검사하지 않습니다.
  `none — <이유>` 로 채워진 줄이 사용자 눈에 그대로 보인다는 것이 이 장치의 값어치입니다.
  여기서 유일하게 검사가 없는 약속입니다.
...
- **세션 탐지는 추론입니다.** 확장에서 실행되는 세션은 프로세스 이름이 `claude` 가 아니고,
  다른 디렉터리에서 이 트리를 고치는 세션은 여기에 흔적을 남기지 않습니다. lease 가
  그 틈을 대부분 메우지만 최근에 도구를 호출한 세션에 한합니다. 탐지가 불가능한
  환경에서는 가드가 스스로 판정하지 않고 물어봅니다.
- **Windows 는 실기기에서 검증하지 않았습니다.** 훅이 shebang 과 실행 비트에 의존하지 않고
  `python3` 로 명시 실행되므로 동작할 것으로 보지만, 실제로 Windows 에서 돌려본 사람은
  없습니다.
- **적합성 eval 은 한 번도 실행되지 않았습니다.** `evals/` 는 early access 기능인
  `claude plugin eval` 을 대상으로 작성했고 활성화를 기다리는 중입니다. 통과한 것이 아니라
  작성만 된 상태로 보시면 됩니다.
